...
void send_mbhc_impedance_to_xlog(const unsigned int zl, const unsigned int zr)
{
	...
	ret = wcd938x_xlog_send_int(zl, zr);
	if (ret < 0) {
...
int wcd938x_xlog_send_int(const unsigned int zl, const unsigned int zr)
{
	int ret = 0;
	char msg[512];
	pr_info("%s: zl = %d, zr = %d", __func__, zl, zr);
	ret = wcd938x_xlog_format_msg_int(msg, zl, zr);
	if (ret < 0) {
		return ret;
	}
...
int wcd938x_xlog_format_msg_int (char *msg, const unsigned int zl, const unsigned int zr)
{
	if (msg == NULL) {
