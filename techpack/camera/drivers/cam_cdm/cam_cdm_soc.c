...
bool cam_cdm_read_hw_reg(struct cam_hw_info *cdm_hw, enum cam_cdm_regs reg,
			 uint32_t *value)
{
	...
	CAM_DBG(CAM_CDM, "E: b=%pK reg=%x off=%x", (void __iomem *)base, reg,
		(CAM_CDM_OFFSET_FROM_REG(cdm->offset_tbl, reg)));
	...
	if ((reg > cdm->offset_tbl->offset_max_size) ||
		...
		if (reg_addr > (base + mem_len)) {
			CAM_ERR_RATE_LIMIT(CAM_CDM, "Invalid mapped region %d",
					   reg);
			goto permission_error;
		}
...
bool cam_cdm_write_hw_reg(struct cam_hw_info *cdm_hw, enum cam_cdm_regs reg,
			  uint32_t value)
{
	void __iomem *reg_addr;
	struct cam_cdm *cdm = (struct cam_cdm *)cdm_hw->core_info;
	...
	if ((reg > cdm->offset_tbl->offset_max_size) ||
		...
		reg_addr =
			(base + CAM_CDM_OFFSET_FROM_REG(cdm->offset_tbl, reg));
		if (reg_addr > (base + mem_len)) {
			CAM_ERR_RATE_LIMIT(CAM_CDM,
					   "Accessing invalid region %d:%d\n",
					   reg,
					   (CAM_CDM_OFFSET_FROM_REG(
						   cdm->offset_tbl, reg)));
			goto permission_error;
		}
				...
				struct cam_cdm_private_dt_data *ptr)
{
	...
	for (i = 0; i < ptr->dt_num_supported_clients; i++) {
		rc = of_property_read_string_index(
			pdev->dev.of_node, "cdm-client-names", i,
...
int cam_cdm_intf_mgr_soc_get_dt_properties(struct platform_device *pdev,
					   struct cam_cdm_intf_mgr *mgr)
{
	int rc;

	rc = of_property_read_u32(pdev->dev.of_node, "num-hw-cdm",
