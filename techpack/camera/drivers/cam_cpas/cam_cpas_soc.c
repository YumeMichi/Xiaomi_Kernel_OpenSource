...
void cam_cpas_dump_axi_vote_info(const struct cam_cpas_client *cpas_client,
				 ...
				 struct cam_axi_vote *axi_vote)
{
	...
	for (i = 0; i < axi_vote->num_paths; i++) {
		CAM_INFO(
			...
			cpas_client->data.cell_index, identifier,
...
void cam_cpas_util_debug_parse_data(struct cam_cpas_private_soc *soc_private)
{
	...
	for (i = 0; i < CAM_CPAS_MAX_TREE_NODES; i++) {
		...
		CAM_INFO(
			...
			curr_node->merge_type,
			curr_node->parent_node ?
				curr_node->parent_node->node_name :
				"no parent");
				    ...
				    struct cam_cpas_private_soc *soc_private)
{
	...
	for_each_available_child_of_node (camera_bus_node, level_node) {
		...
		camnoc_max_needed =
			of_property_read_bool(level_node, "camnoc-max-needed");

		for_each_available_child_of_node (level_node, curr_node) {
			curr_node_ptr = kzalloc(
				sizeof(struct cam_cpas_tree_node), GFP_KERNEL);
			if (!curr_node_ptr)
				return -ENOMEM;
			...
			if (rc) {
				CAM_ERR(CAM_CPAS,
					"failed to read node-name rc=%d", rc);
				return rc;
			}

			curr_node_ptr->camnoc_max_needed = camnoc_max_needed;
			rc = of_property_read_u32(
				curr_node, "bus-width-factor",
			...
			if (!rc) {
				...
				rc = of_property_read_u32(
					curr_node, "traffic-transaction-type",
				...
				count = of_property_count_u32_elems(
				...
				for (i = 0; i < count; i++) {
					...
					curr_node_ptr
						->constituent_paths[path_idx] =
						true;
				}

				rc = cam_common_util_get_string_index(
					soc_private->client_name,
					soc_private->num_clients, client_name,
					&client_idx);
				if (rc) {
					CAM_ERR(CAM_CPAS,
				...
				curr_client->tree_node
					...
					[curr_node_ptr->path_trans_type] =
					curr_node_ptr;
				CAM_DBG(CAM_CPAS, "CLIENT NODE ADDED: %d %d %s",
					curr_node_ptr->path_data_type,
					curr_node_ptr->path_trans_type,
					client_name);
			}
			...
			parent_node =
				of_parse_phandle(curr_node, "parent-node", 0);
			if (parent_node) {
				...
				struct cam_cpas_private_soc *soc_private)
{
	...
	if (!soc_private || !pdev) {
		CAM_ERR(CAM_CPAS, "invalid input arg %pK %pK", soc_private,
			pdev);
		return -EINVAL;
	}
	...
	if (rc == -EOVERFLOW) {
		soc_private->camnoc_axi_min_ib_bw = 0;
		rc = of_property_read_u32(
			of_node, "camnoc-axi-min-ib-bw",
			(u32 *)&soc_private->camnoc_axi_min_ib_bw);
	}

	if (rc) {
		CAM_DBG(CAM_CPAS, "failed to read camnoc-axi-min-ib-bw rc:%d",
			rc);
		soc_private->camnoc_axi_min_ib_bw =
	...
	soc_private->client_id_based =
		of_property_read_bool(of_node, "client-id-based");
	...
	soc_private->control_camnoc_axi_clk =
		of_property_read_bool(of_node, "control-camnoc-axi-clk");
	...
	if (soc_private->control_camnoc_axi_clk == true) {
		...
		if (rc || (soc_private->camnoc_bus_width == 0)) {
			CAM_ERR(CAM_CPAS, "Bus width not found rc=%d, %d", rc,
				soc_private->camnoc_bus_width);
			goto cleanup_clients;
		}

		rc = of_property_read_u32(
			of_node, "camnoc-axi-clk-bw-margin-perc",
	...
	CAM_DBG(CAM_CPAS, "control_camnoc_axi_clk=%d, width=%d, margin=%d",
		soc_private->control_camnoc_axi_clk,
		soc_private->camnoc_bus_width,
		soc_private->camnoc_axi_clk_bw_margin);
	...
	if ((count > 0) && (count <= CAM_REGULATOR_LEVEL_MAX) &&
		...
		for (i = 0; i < count; i++) {
			...
			rc = of_property_read_string_index(
				of_node, "vdd-corner-ahb-mapping", i,
				&ahb_string);
			if (rc) {
				CAM_ERR(CAM_CPAS, "no ahb-mapping at index=%d",
					i);
				rc = -ENODEV;
				goto cleanup_clients;
			}

			rc = cam_soc_util_get_level_from_string(
			...
			CAM_DBG(CAM_CPAS,
				...
				soc_private->vdd_ahb[i].vdd_corner, ahb_string,
				...
				struct cam_hw_info *cpas_hw)
{
	...
	soc_info->soc_private =
		kzalloc(sizeof(struct cam_cpas_private_soc), GFP_KERNEL);
				   ...
				   bool disable_clocks, bool disable_irq)
{
	...
	rc = cam_soc_util_disable_platform_resource(soc_info, disable_clocks,
						    disable_irq);
	if (rc)
