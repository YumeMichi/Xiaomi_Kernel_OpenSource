...
static int
cam_cpastop_handle_errlogger(struct cam_cpas *cpas_core,
			     ...
			     struct cam_camnoc_irq_slave_err_data *slave_err)
{
	...
	CAM_ERR_RATE_LIMIT(
		...
		camnoc_info->err_logger->mainctrl, slave_err->mainctrl.value,
		camnoc_info->err_logger->errvld, slave_err->errvld.value,
		slave_err->mainctrl.stall_en, slave_err->mainctrl.fault_en,
	...
	CAM_ERR_RATE_LIMIT(
		...
		slave_err->errlog0_low.device, slave_err->errlog0_low.opc,
		slave_err->errlog0_low.err_code,
		camnoc_salve_err_code[err_code_index],
		slave_err->errlog0_low.sizef, slave_err->errlog0_low.addr_space,
...
static int
cam_cpastop_handle_ubwc_enc_err(struct cam_cpas *cpas_core,
				...
				struct cam_camnoc_irq_ubwc_enc_data *enc_err)
{
	...
	CAM_DBG(CAM_CPAS, "ubwc enc err [%d]: offset[0x%x] value[0x%x]", i,
		camnoc_info->irq_err[i].err_status.offset,
		enc_err->encerr_status.value);
...
static int
cam_cpastop_handle_ubwc_dec_err(struct cam_cpas *cpas_core,
				struct cam_hw_soc_info *soc_info, int i,
				struct cam_camnoc_irq_ubwc_dec_data *dec_err)
{
	...
	CAM_DBG(CAM_CPAS,
		...
		dec_err->decerr_status.value, dec_err->decerr_status.thr_err,
...
static int cam_cpastop_handle_ahb_timeout_err(
	...
	for (i = 0; i < cpas_core->num_clients; i++) {
		if (CAM_CPAS_CLIENT_STARTED(cpas_core, i)) {
			...
			if (cpas_client->data.cam_cpas_client_cb) {
				CAM_DBG(CAM_CPAS, "Calling client CB %d : %d",
					i, irq_data->irq_type);
				error_handled =
...
static void cam_cpastop_work(struct work_struct *work)
{
	...
	for (i = 0; i < camnoc_info->irq_err_size; i++) {
		if ((payload->irq_status & camnoc_info->irq_err[i].sbm_port) &&
			...
			CAM_ERR_RATE_LIMIT(CAM_CPAS, "Error occurred, type=%d",
					   irq_type);
...
static int cam_cpastop_poweroff(struct cam_hw_info *cpas_hw)
{
	...
	if (errata_wa_list->camnoc_flush_slave_pending_trans.enable) {
		...
		rc = cam_io_poll_value_wmask(
			...
			errata_wa->data.reg_info.mask, CAM_CPAS_POLL_RETRY_CNT,
				       ...
				       struct cam_cpas_hw_caps *hw_caps)
{
	...
	CAM_DBG(CAM_CPAS,
		...
		soc_info->hw_version, hw_caps->camera_version.major,
		hw_caps->camera_version.minor, hw_caps->camera_version.incr,
		hw_caps->cpas_version.major, hw_caps->cpas_version.minor,
		hw_caps->cpas_version.incr);
