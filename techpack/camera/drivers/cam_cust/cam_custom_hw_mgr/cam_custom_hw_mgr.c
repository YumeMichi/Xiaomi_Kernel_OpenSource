...
static int cam_custom_mgr_get_hw_caps(void *hw_mgr_priv, void *hw_caps_args)
{
	...
	cam_custom_hw =
		(struct cam_hw_info *)g_custom_hw_mgr.custom_hw[0]->hw_priv;
	if (cam_custom_hw)
...
cam_custom_hw_mgr_get_custom_res_state(uint32_t in_rsrc_state)
{
...
cam_custom_hw_mgr_get_isp_res_state(uint32_t in_rsrc_state)
{
...
cam_custom_hw_mgr_get_isp_res_type(enum cam_custom_hw_mgr_res_type res_type)
{
	switch (res_type) {
...
static int
cam_custom_hw_mgr_deinit_hw_res(struct cam_custom_hw_mgr_res *hw_mgr_res)
{
	int rc = -1;
...
static int
cam_custom_hw_mgr_stop_hw_res(struct cam_custom_hw_mgr_res *hw_mgr_res)
{
	int rc = -1;
...
err:
	CAM_DBG(CAM_CUSTOM, "STOP HW failed for res_id:%u", hw_mgr_res->res_id);
	return rc;
}
...
static int cam_custom_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	ctx = (struct cam_custom_hw_mgr_ctx *)stop_args->ctxt_to_hw_map;
	...
	list_for_each_entry (hw_mgr_res, &ctx->res_list_custom_cid, list) {
...
static int
cam_custom_hw_mgr_init_hw_res(struct cam_custom_hw_mgr_res *hw_mgr_res)
{
	int rc = -1;
	struct cam_isp_resource_node *isp_rsrc_node = NULL;
	struct cam_hw_intf *hw_intf = NULL;
	...
	if (hw_intf->hw_ops.init) {
		CAM_DBG(CAM_CUSTOM, "INIT HW for res_id:%u",
			hw_mgr_res->res_id);
		rc = hw_intf->hw_ops.init(hw_intf->hw_priv, isp_rsrc_node,
					  sizeof(struct cam_isp_resource_node));
		if (rc)
			goto err;
	}
...
err:
	CAM_DBG(CAM_CUSTOM, "INIT HW failed for res_id:%u", hw_mgr_res->res_id);
	return rc;
}

static int
cam_custom_hw_mgr_start_hw_res(struct cam_custom_hw_mgr_res *hw_mgr_res)
{
	int rc = -1;
	struct cam_isp_resource_node *isp_rsrc_node = NULL;
	struct cam_hw_intf *hw_intf = NULL;
...
static int cam_custom_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	int rc = 0;
	...
	ctx = (struct cam_custom_hw_mgr_ctx *)hw_config->ctxt_to_hw_map;
	if (!ctx || !ctx->ctx_in_use) {
		CAM_ERR(CAM_CUSTOM, "Invalid context is used");
		return -EPERM;
	}
...
static int cam_custom_hw_mgr_put_ctx(struct list_head *src_list,
...
static int cam_custom_hw_mgr_get_ctx(struct list_head *src_list,
				     struct cam_custom_hw_mgr_ctx **custom_ctx)
{
	struct cam_custom_hw_mgr_ctx *ctx_ptr = NULL;
	...
	*custom_ctx = ctx_ptr;
	memset(ctx_ptr->sub_hw_list, 0,
	       sizeof(struct cam_custom_hw_mgr_res) * CAM_CUSTOM_HW_RES_MAX);

	return 0;
}

static int cam_custom_hw_mgr_put_res(struct list_head *src_list,
...
static int cam_custom_hw_mgr_get_res(struct list_head *src_list,
				     struct cam_custom_hw_mgr_res **res)
{
	int rc = 0;
	struct cam_custom_hw_mgr_res *res_ptr = NULL;
...
cam_custom_hw_mgr_get_csid_res_type(uint32_t out_port_type)
{
...
static int
cam_custom_hw_mgr_acquire_cid_res(struct cam_custom_hw_mgr_ctx *custom_ctx,
				  ...
				  struct cam_isp_resource_node **cid_rsrc_node)
{
	...
	*cid_res = NULL;
	...
	rc = cam_custom_hw_mgr_get_res(&custom_ctx->free_res_list, cid_res);
	if (rc) {
	...
	struct cam_isp_in_port_generic_info *in_port_info)
{
	...
	for (i = 0; i < in_port_info->num_out_res; i++) {
		...
		path_res_id =
			cam_custom_hw_mgr_get_csid_res_type(out_port->res_type);
		...
		cam_custom_hw_mgr_put_res(&custom_ctx->res_list_custom_csid,
					  &custom_csid_res);
...
put_res:
	cam_custom_hw_mgr_put_res(&custom_ctx->free_res_list, &custom_csid_res);
end:
	return rc;
}

static int
cam_custom_hw_mgr_free_hw_res(struct cam_custom_hw_mgr_res *hw_mgr_res)
{
	int rc = 0;
	struct cam_isp_resource_node *isp_rsrc_node = NULL;
	struct cam_hw_intf *hw_intf = NULL;
	...
	if (hw_intf->hw_ops.release) {
		...
		rc = hw_intf->hw_ops.release(
			...
			sizeof(struct cam_isp_resource_node));
		if (rc)
			CAM_ERR(CAM_CUSTOM, "Release HW failed for hw_idx %d",
				hw_intf->hw_idx);
	}
...
static int
cam_custom_hw_mgr_release_hw_for_ctx(struct cam_custom_hw_mgr_ctx *custom_ctx)
{
	int rc = -1;
	struct cam_custom_hw_mgr_res *hw_mgr_res;
...
static int cam_custom_mgr_release_hw(void *hw_mgr_priv, void *release_hw_args)
{
	int rc = 0;
	...
	CAM_DBG(CAM_CUSTOM, "Enter...ctx id:%d", custom_ctx->ctx_index);
	...
	struct cam_isp_in_port_generic_info *gen_port_info)
{
	...
	gen_port_info->res_type =
		in->res_type + CAM_ISP_IFE_IN_RES_BASE - CAM_CUSTOM_IN_RES_BASE;
	...
	uint32_t *acquired_hw_id, uint32_t *acquired_hw_path)
{
	...
	for (i = 0; i < CAM_CUSTOM_HW_SUB_MOD_MAX; i++) {
		...
		rc = hw_intf->hw_ops.reserve(hw_intf->hw_priv, &acq,
					     sizeof(acq));
		if (rc) {
			CAM_DBG(CAM_CUSTOM, "No custom resource from hw %d",
				hw_intf->hw_idx);
			continue;
		}
...
static int cam_custom_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	int rc = -1;
	int32_t i;
	...
	rc = cam_custom_hw_mgr_get_ctx(&custom_hw_mgr->free_ctx_list,
				       &custom_ctx);
	if (rc || !custom_ctx) {
	...
	custom_rsrc = kcalloc(acquire_args->num_acq, sizeof(*custom_rsrc),
			      GFP_KERNEL);
	if (!custom_rsrc) {
		rc = -ENOMEM;
		goto free_ctx;
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		CAM_DBG(CAM_CUSTOM,
			...
			custom_rsrc[i].res_hdl, custom_rsrc[i].length);
		...
		in_port_info =
			memdup_user(u64_to_user_ptr(custom_rsrc[i].res_hdl),
		...
		if (!IS_ERR(in_port_info)) {
			...
			rc = cam_custom_mgr_acquire_hw_for_ctx(
				custom_ctx, gen_port_info,
...
static int cam_custom_add_io_buffers(int iommu_hdl,
				     struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < prepare->packet->num_io_configs; i++) {
		...
		if (io_cfg[i].direction == CAM_BUF_OUTPUT) {
			CAM_DBG(CAM_CUSTOM, "output fence 0x%x",
				io_cfg[i].fence);
		} else if (io_cfg[i].direction == CAM_BUF_INPUT) {
			CAM_DBG(CAM_CUSTOM, "input fence 0x%x",
				io_cfg[i].fence);
		} else {
		...
		for (plane_id = 0; plane_id < CAM_PACKET_MAX_PLANES;
			...
			CAM_DBG(CAM_CUSTOM, "handle 0x%x for plane %d", hdl,
				plane_id);
					    ...
					    void *prepare_hw_update_args)
{
	...
	if (!rc) {
		...
		custom_buf_type1 = (struct cam_custom_cmd_buf_type_1 *)ptr;
...
static int cam_custom_mgr_config_hw(void *hw_mgr_priv, void *hw_config_args)
{
	int rc = 0;
	int i = 0;
	struct cam_custom_hw_mgr_ctx *custom_ctx;
	...
	cfg = (struct cam_hw_config_args *)hw_config_args;
		...
		if (res->hw_res) {
			...
			if (hw_intf->hw_ops.process_cmd) {
				...
				rc = hw_intf->hw_ops.process_cmd(
					hw_intf->hw_priv, CAM_CUSTOM_SUBMIT_REQ,
				    ...
				    struct cam_custom_hw_cb_args *cb_args)
{
	...
	custom_ctx->event_cb(custom_ctx->cb_priv, CAM_CUSTOM_EVENT_BUF_DONE,
			     &evt_data);
			   ...
			   struct cam_hw_mgr_intf *hw_mgr_intf, int *iommu_hdl)
{
	...
	for (i = 0; i < CAM_CUSTOM_HW_SUB_MOD_MAX; i++) {
		...
		rc = cam_custom_hw_sub_mod_init(&g_custom_hw_mgr.custom_hw[i],
						i);
		...
		if (g_custom_hw_mgr.custom_hw[i]) {
			...
			if (hw_intf->hw_ops.process_cmd) {
				...
				hw_intf->hw_ops.process_cmd(
					hw_intf->hw_priv, CAM_CUSTOM_SET_IRQ_CB,
	...
	for (i = 0; i < CAM_CUSTOM_CSID_HW_MAX; i++) {
		...
		rc = cam_custom_csid_hw_init(&g_custom_hw_mgr.csid_devices[i],
					     i);
	}
	 ...
	 *  deinit support
	 */
	if (cam_smmu_get_handle("custom", &g_custom_hw_mgr.img_iommu_hdl)) {
