...
int cam_a5_init_hw(void *device_priv, void *init_hw_args, uint32_t arg_size)
{
	struct cam_hw_info *a5_dev = device_priv;
	struct cam_hw_soc_info *soc_info = NULL;
	struct cam_a5_device_core_info *core_info = NULL;
	...
	cpas_vote.axi_vote.axi_path[0].camnoc_bw = CAM_ICP_A5_BW_BYTES_VOTE;
	cpas_vote.axi_vote.axi_path[0].mnoc_ab_bw = CAM_ICP_A5_BW_BYTES_VOTE;
	cpas_vote.axi_vote.axi_path[0].mnoc_ib_bw = CAM_ICP_A5_BW_BYTES_VOTE;
	cpas_vote.axi_vote.axi_path[0].ddr_ab_bw = CAM_ICP_A5_BW_BYTES_VOTE;
	cpas_vote.axi_vote.axi_path[0].ddr_ib_bw = CAM_ICP_A5_BW_BYTES_VOTE;
...
int cam_a5_deinit_hw(void *device_priv, void *init_hw_args, uint32_t arg_size)
{
	struct cam_hw_info *a5_dev = device_priv;
	struct cam_hw_soc_info *soc_info = NULL;
	struct cam_a5_device_core_info *core_info = NULL;
...
irqreturn_t cam_a5_irq(int irq_num, void *data)
{
	...
	cam_io_w_mb(irq_status, soc_info->reg_map[A5_SIERRA_BASE].mem_base +
					core_info->a5_hw_info->a5_host_int_clr);

	if ((irq_status & A5_WDT_0) || (irq_status & A5_WDT_1)) {
...
int cam_a5_process_cmd(void *device_priv, uint32_t cmd_type, void *cmd_args,
		       uint32_t arg_size)
{
	struct cam_hw_info *a5_dev = device_priv;
	struct cam_hw_soc_info *soc_info = NULL;
