			     ...
			     struct cam_bps_device_core_info *core_info)
{
	...
	while (retry_cnt < HFI_MAX_POLL_TRY) {
		readw_poll_timeout(
			(soc_info->reg_map[0].mem_base + BPS_CDM_IRQ_STATUS),
	...
	status =
		cam_io_r_mb(soc_info->reg_map[0].mem_base + BPS_CDM_IRQ_STATUS);
	if ((status & BPS_RST_DONE_IRQ_STATUS_BIT) != 0x1) {
	...
	while (retry_cnt < HFI_MAX_POLL_TRY) {
		readw_poll_timeout(
			(soc_info->reg_map[0].mem_base + BPS_TOP_IRQ_STATUS),
			status, ((status & BPS_RST_DONE_IRQ_STATUS_BIT) == 0x1),
			100, 10000);
	...
	status =
		cam_io_r_mb(soc_info->reg_map[0].mem_base + BPS_TOP_IRQ_STATUS);
	if ((status & BPS_RST_DONE_IRQ_STATUS_BIT) != 0x1) {
