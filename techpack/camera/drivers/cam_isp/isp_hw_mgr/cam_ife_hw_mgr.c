...
static uint32_t blob_type_hw_cmd_map[CAM_ISP_GENERIC_BLOB_TYPE_MAX] = {
	CAM_ISP_HW_CMD_GET_HFR_UPDATE,	  CAM_ISP_HW_CMD_CLOCK_UPDATE,
	CAM_ISP_HW_CMD_BW_UPDATE,	  CAM_ISP_HW_CMD_UBWC_UPDATE,
	CAM_ISP_HW_CMD_CSID_CLOCK_UPDATE, CAM_ISP_GENERIC_BLOB_TYPE_FE_CONFIG,
	CAM_ISP_HW_CMD_UBWC_UPDATE_V2,	  CAM_ISP_HW_CMD_CORE_CONFIG,
	CAM_ISP_HW_CMD_WM_CONFIG_UPDATE,  CAM_ISP_HW_CMD_BW_UPDATE_V2,
};

static struct cam_ife_hw_mgr g_ife_hw_mgr;

static int cam_ife_hw_mgr_event_handler(void *priv, uint32_t evt_id,
					void *evt_info);
	...
	*soc_info_ptr = NULL;
	list_for_each_entry_safe (hw_mgr_res, hw_mgr_res_temp,
		...
		case CAM_REG_DUMP_BASE_TYPE_ISP_LEFT:
			...
			rc = hw_mgr_res->hw_res[CAM_ISP_HW_SPLIT_LEFT]->process_cmd(
		...
		case CAM_REG_DUMP_BASE_TYPE_ISP_RIGHT:
			...
			rc = hw_mgr_res->hw_res[CAM_ISP_HW_SPLIT_RIGHT]
				     ->process_cmd(
					     hw_mgr_res->hw_res
			...
			*soc_info_ptr = soc_info;
			break;
		default:
			CAM_ERR(CAM_ISP, "Unrecognized reg base type: %u",
				reg_base_type);
			return -EINVAL;
		}
...
static int
cam_ife_mgr_handle_reg_dump(struct cam_ife_hw_mgr_ctx *ctx,
			    struct cam_cmd_buf_desc *reg_dump_buf_desc,
			    uint32_t num_reg_dump_buf, uint32_t meta_type)
{
	int rc = 0, i;
	...
	for (i = 0; i < num_reg_dump_buf; i++) {
		...
		if (reg_dump_buf_desc[i].meta_data == meta_type) {
			rc = cam_soc_util_reg_dump_to_cmd_buf(
				ctx, &reg_dump_buf_desc[i], ctx->applied_req_id,
...
static int cam_ife_mgr_get_hw_caps(void *hw_mgr_priv, void *hw_caps_args)
{
	...
	if (copy_to_user(u64_to_user_ptr(query->caps_handle), &query_isp,
			 sizeof(struct cam_isp_query_cap_cmd)))
		rc = -EFAULT;
...
static void cam_ife_hw_mgr_deinit_hw(struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_ife_hw_mgr_res *hw_mgr_res;
	int i = 0;
...
static int cam_ife_hw_mgr_init_hw(struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_ife_hw_mgr_res *hw_mgr_res;
	int rc = 0, i;
	...
	/* INIT IFE BUS RD */
	CAM_DBG(CAM_ISP, "INIT IFE BUS RD in ctx id:%d", ctx->ctx_index);
	list_for_each_entry (hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		rc = cam_ife_hw_mgr_init_hw_res(hw_mgr_res);
		if (rc) {
...
static const char *cam_ife_hw_mgr_get_res_state(uint32_t res_state)
{
	switch (res_state) {
...
static const char *cam_ife_hw_mgr_get_csid_res_id(uint32_t res_id)
{
	switch (res_id) {
...
static const char *cam_ife_hw_mgr_get_src_res_id(uint32_t res_id)
{
	switch (res_id) {
...
static void
cam_ife_hw_mgr_dump_src_acq_info(struct cam_ife_hw_mgr_ctx *hwr_mgr_ctx,
...
static void cam_ife_hw_mgr_dump_acq_data(struct cam_ife_hw_mgr_ctx *hwr_mgr_ctx)
{
	struct cam_ife_hw_mgr_res *hw_mgr_res = NULL;
	struct cam_ife_hw_mgr_res *hw_mgr_res_temp = NULL;
	struct cam_isp_resource_node *hw_res = NULL;
	...
	CAM_INFO(
		...
		hrs, min, sec, ms, hwr_mgr_ctx->ctx_index,
...
static int
cam_ife_hw_mgr_acquire_res_bus_rd(struct cam_ife_hw_mgr_ctx *ife_ctx,
...
static int cam_ife_hw_mgr_acquire_res_ife_out_pixel(
	struct cam_ife_hw_mgr_ctx *ife_ctx,
	struct cam_ife_hw_mgr_res *ife_src_res,
	struct cam_isp_in_port_generic_info *in_port, bool acquire_lcr)
{
	int rc = -1;
	uint32_t i, j, k;
	struct cam_vfe_acquire_args vfe_acquire;
	...
	list_for_each_entry (ife_src_res, &ife_ctx->res_list_ife_src, list) {
		...
		case CAM_ISP_HW_VFE_IN_RD:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_pixel(
				ife_ctx, ife_src_res, in_port, false);
			break;
		case CAM_ISP_HW_VFE_IN_LCR:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_pixel(
				ife_ctx, ife_src_res, in_port, true);
			break;
		case CAM_ISP_HW_VFE_IN_RDI0:
		case CAM_ISP_HW_VFE_IN_RDI1:
		case CAM_ISP_HW_VFE_IN_RDI2:
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	list_for_each_entry (csid_res, &ife_ctx->res_list_ife_csid, list) {
		...
		hw_intf = ife_hw_mgr->ife_devices
				  [csid_res->hw_res[CAM_ISP_HW_SPLIT_LEFT]
					   ->hw_intf->hw_idx];

		vfe_idx =
			csid_res->hw_res[CAM_ISP_HW_SPLIT_LEFT]->hw_intf->hw_idx;
		...
		rc = hw_intf->hw_ops.reserve(
			...
			sizeof(struct cam_vfe_acquire_args));
		if (rc) {
			CAM_ERR(CAM_ISP, "Can not acquire IFE HW res %d",
				csid_res->res_id);
			goto err;
		}
...
static int
cam_ife_mgr_acquire_cid_res(struct cam_ife_hw_mgr_ctx *ife_ctx,
			    struct cam_isp_in_port_generic_info *in_port,
...
acquire_successful:
	 ...
	 */
	if (cid_res_temp->is_dual_vfe &&
	    path_res_id == CAM_IFE_PIX_PATH_RES_IPP && acquired_cnt == 1) {
...
static int cam_ife_hw_mgr_acquire_res_ife_csid_pxl(
	struct cam_ife_hw_mgr_ctx *ife_ctx,
	struct cam_isp_in_port_generic_info *in_port, bool is_ipp,
	bool crop_enable)
{
	int rc = -1;
	int i;
	...
	for (i = 0; i <= csid_res->is_dual_vfe; i++) {
		...
		if (csid_res->is_dual_vfe) {
			if (i == CAM_ISP_HW_SPLIT_LEFT) {
				...
				if (master_idx == -1) {
					CAM_ERR(CAM_ISP, "No Master found");
					goto put_res;
				}
...
static int
cam_ife_hw_mgr_acquire_res_root(struct cam_ife_hw_mgr_ctx *ife_ctx,
				struct cam_isp_in_port_generic_info *in_port)
{
	int rc = -1;
	...
	struct cam_isp_acquire_hw_info *acquire_hw_info)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		CAM_DBG(CAM_ISP, "in_port%d res_type %d", i, in_port->res_type);
	...
	struct cam_isp_acquire_hw_info *acquire_hw_info)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		in_port_length = sizeof(struct cam_isp_in_port_info_v2) +
				 (in_port->num_out_res -
				  1) * sizeof(struct cam_isp_out_port_info_v2);
		total_in_port_length += in_port_length;
		...
		in_port =
			(struct cam_isp_in_port_info_v2 *)((uint8_t *)in_port +
							   in_port_length);
	}
	CAM_DBG(CAM_ISP, "is_fe_enable %d", ife_ctx->is_fe_enable);
...
static int
cam_ife_mgr_check_and_update_fe(struct cam_ife_hw_mgr_ctx *ife_ctx,
				struct cam_isp_acquire_hw_info *acquire_hw_info)
{
	...
	switch (major_ver) {
	case 1:
		return cam_ife_mgr_check_and_update_fe_v0(ife_ctx,
							  acquire_hw_info);
	case 2:
		return cam_ife_mgr_check_and_update_fe_v2(ife_ctx,
							  acquire_hw_info);
		break;
	default:
...
static int
cam_ife_hw_mgr_preprocess_port(struct cam_ife_hw_mgr_ctx *ife_ctx,
			       struct cam_isp_in_port_generic_info *in_port,
			       int *ipp_count, int *rdi_count, int *ppp_count,
			       int *ife_rd_count, int *lcr_count)
{
	int ipp_num = 0;
	int rdi_num = 0;
	int ppp_num = 0;
	...
	*lcr_count = lcr_num;
	...
	CAM_DBG(CAM_ISP, "rdi: %d ipp: %d ppp: %d ife_rd: %d lcr: %d", rdi_num,
		ipp_num, ppp_num, ife_rd_num, lcr_num);

	return 0;
}

static int
cam_ife_mgr_acquire_hw_for_ctx(struct cam_ife_hw_mgr_ctx *ife_ctx,
			       struct cam_isp_in_port_generic_info *in_port,
			       uint32_t *num_pix_port, uint32_t *num_rdi_port,
			       ...
			       uint32_t *acquired_hw_path)
{
	...
	cam_ife_hw_mgr_preprocess_port(ife_ctx, in_port, &ipp_count, &rdi_count,
				       &ppp_count, &ife_rd_count, &lcr_count);
	...
	if (ipp_count || lcr_count) {
		...
		rc = cam_ife_hw_mgr_acquire_res_ife_csid_pxl(ife_ctx, in_port,
							     true, crop_enable);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	if (ppp_count) {
		...
		rc = cam_ife_hw_mgr_acquire_res_ife_csid_pxl(
			ife_ctx, in_port, false, crop_enable);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	if (ife_rd_count) {
		...
		rc = cam_ife_hw_mgr_acquire_res_ife_src(ife_ctx, in_port, false,
							acquired_hw_id,
							acquired_hw_path);
	...
	if (status == CAM_CDM_CB_STATUS_BL_SUCCESS) {
		...
		if (g_ife_hw_mgr.debug_cfg.per_req_reg_dump)
			cam_ife_mgr_handle_reg_dump(
				ctx, hw_update_data->reg_dump_buf_desc,
...
static int cam_ife_mgr_acquire_get_unified_structure_v0(
	struct cam_isp_acquire_hw_info *acquire_hw_info, uint32_t offset,
	uint32_t *input_size, struct cam_isp_in_port_generic_info **in_port)
{
	...
	in = (struct cam_isp_in_port_info *)((uint8_t *)&acquire_hw_info->data +
					     acquire_hw_info->input_info_offset +
					     *input_size);

	in_port_length =
		sizeof(struct cam_isp_in_port_info) +
		(in->num_out_res - 1) * sizeof(struct cam_isp_out_port_info);
	...
	*input_size += in_port_length;
	...
	port_info = kzalloc(sizeof(struct cam_isp_in_port_generic_info),
			    GFP_KERNEL);
	...
	port_info->minor_ver = acquire_hw_info->input_info_version & 0xFFFF;
	...
	uint32_t *input_size, struct cam_isp_in_port_generic_info **in_port)
{
	...
	in_port_length =
		...
		(in->num_out_res - 1) * sizeof(struct cam_isp_out_port_info_v2);
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		rc = cam_ife_mgr_acquire_get_unified_structure(
			acquire_hw_info, i, &input_size, &in_port);
		if (rc < 0) {
		...
		rc = cam_ife_mgr_acquire_hw_for_ctx(
			...
			&num_rdi_port_per_in, &acquire_args->acquired_hw_id[i],
		...
		if (rc) {
			...
			cam_ife_hw_mgr_dump_src_acq_info(
				ife_ctx, total_pix_port, total_rdi_port);
			goto free_mem;
		}
	...
	acquire_args->valid_acquired_hw = acquire_hw_info->num_inputs;

	getnstimeofday64(&ife_ctx->ts);
	CAM_INFO(
		CAM_ISP,
		"Acquire HW success with total_pix: %u total_rdi: %u is_dual: %u in ctx: %u",
		total_pix_port, total_rdi_port, ife_ctx->is_dual,
		ife_ctx->ctx_index);
...
void cam_ife_mgr_acquire_get_unified_dev_str(
	struct cam_isp_in_port_info *in,
...
static int cam_ife_mgr_acquire_dev(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		in_port = memdup_user(u64_to_user_ptr(isp_resource[i].res_hdl),
		...
		if (!IS_ERR(in_port)) {
			...
			rc = cam_ife_mgr_acquire_hw_for_ctx(
				ife_ctx, gen_port_info, &num_pix_port_per_in,
				&num_rdi_port_per_in,
...
/* entry function: acquire_hw */
static int cam_ife_mgr_acquire(void *hw_mgr_priv, void *acquire_hw_args)
{
	struct cam_hw_acquire_args *acquire_args = acquire_hw_args;
	int rc = -1;
...
static const char *cam_isp_util_usage_data_to_string(uint32_t usage_data)
{
	switch (usage_data) {
...
static int cam_isp_classify_vote_info(struct cam_ife_hw_mgr_res *hw_mgr_res,
		...
		for (i = 0; i < bw_config->num_paths; i++) {
			if ((bw_config->axi_path[i].usage_data ==
			    ...
			    ((bw_config->axi_path[i].path_data_type -
			     ...
			     (hw_mgr_res->res_id - CAM_ISP_HW_VFE_IN_RDI0))) {
				memcpy(&isp_vote->axi_path[j],
				       &bw_config->axi_path[i],
				       sizeof(struct cam_axi_per_path_bw_vote));
				j++;
...
static int cam_isp_blob_bw_update_v2(struct cam_isp_bw_config_v2 *bw_config,
				     struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_ife_hw_mgr_res *hw_mgr_res;
	struct cam_hw_intf *hw_intf;
		...
		for (split_idx = 0; split_idx < CAM_ISP_HW_SPLIT_MAX;
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				rc = hw_intf->hw_ops.process_cmd(
					...
					&bw_upd_args,
					sizeof(struct cam_vfe_bw_update_args_v2));
				if (rc)
					CAM_ERR(CAM_PERF,
...
static int cam_ife_mgr_config_hw(void *hw_mgr_priv, void *config_hw_args)
{
	...
	CAM_DBG(CAM_ISP, "Ctx[%pK][%d] : Applying Req %lld", ctx,
		ctx->ctx_index, cfg->request_id);

	for (i = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		if (hw_update_data->bw_config_valid[i] == true) {
			CAM_DBG(CAM_PERF, "idx=%d, bw_config_version=%d", ctx,
				ctx->ctx_index, i,
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	cam_ife_hw_mgr_deinit_hw(ctx);
	CAM_DBG(CAM_ISP, "Stop success for ctx id:%d rc :%d", ctx->ctx_index,
		rc);
	...
	list_for_each_entry (hw_mgr_res, &ctx->res_list_ife_src, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (rsrc_node->process_cmd &&
			    (rsrc_node->res_id == CAM_ISP_HW_VFE_IN_CAMIF)) {
				rc = hw_mgr_res->hw_res[i]->process_cmd(
					hw_mgr_res->hw_res[i],
					CAM_ISP_HW_CMD_SET_CAMIF_DEBUG,
					&camif_debug, sizeof(camif_debug));
			}
		}
	}
...
static int cam_ife_mgr_release_hw(void *hw_mgr_priv, void *release_hw_args)
{
	...
	CAM_INFO(CAM_ISP, "Release HW success ctx id: %u", ctx->ctx_index);
...
static int cam_isp_blob_fe_update(uint32_t blob_type,
				  ...
				  struct cam_hw_prepare_update_args *prepare)
{
	...
	list_for_each_entry (hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				fe_upd_args.node_res = hw_mgr_res->hw_res[i];
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				rc = hw_intf->hw_ops.process_cmd(
					...
					&fe_upd_args,
					sizeof(struct cam_vfe_fe_update_args));
				if (rc)
...
static int cam_isp_blob_ubwc_update(uint32_t blob_type,
				    struct cam_isp_generic_blob_info *blob_info,
	...
	case CAM_UBWC_CFG_VERSION_1:
		...
		for (i = 0; i < ubwc_config->num_ports; i++) {
			...
			rc = cam_isp_add_cmd_buf_update(
				...
				kmd_buf_remain_size, (void *)ubwc_plane_cfg,
				&bytes_used);
			if (rc < 0) {
				CAM_ERR(CAM_ISP,
...
static int cam_isp_get_generic_ubwc_data_v2(
	struct cam_ubwc_plane_cfg_v2 *ubwc_cfg, uint32_t version,
	struct cam_vfe_generic_ubwc_config *generic_ubwc_cfg)
{
	...
	for (i = 0; i < CAM_PACKET_MAX_PLANES - 1; i++) {
		...
		generic_ubwc_cfg->ubwc_plane_cfg[i].h_init = ubwc_cfg[i].h_init;
		generic_ubwc_cfg->ubwc_plane_cfg[i].v_init = ubwc_cfg[i].v_init;
		generic_ubwc_cfg->ubwc_plane_cfg[i].static_ctrl =
			ubwc_cfg[i].static_ctrl;
		generic_ubwc_cfg->ubwc_plane_cfg[i].ctrl_2 = ubwc_cfg[i].ctrl_2;
...
static int
cam_isp_blob_ubwc_update_v2(uint32_t blob_type,
			    struct cam_isp_generic_blob_info *blob_info,
			    ...
			    struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < ubwc_config->num_ports; i++) {
		...
		rc = cam_isp_add_cmd_buf_update(
			...
			blob_info->base_info->idx, (void *)cmd_buf_addr,
			kmd_buf_remain_size, (void *)&generic_ubwc_cfg,
			&bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				"Failed cmd_update, base_idx=%d, bytes_used=%u, res_id_out=0x%x",
...
static int
cam_isp_blob_csid_clock_update(uint32_t blob_type,
			       struct cam_isp_generic_blob_info *blob_info,
...
static int
cam_isp_blob_csid_qcfa_update(uint32_t blob_type,
			      struct cam_isp_generic_blob_info *blob_info,
			      ...
			      struct cam_hw_prepare_update_args *prepare)
{
	...
	CAM_DBG(CAM_ISP, "csid binning=%d", qcfa_config->csid_binning);
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				rc = hw_intf->hw_ops.process_cmd(
					...
					&csid_qcfa_upd_args,
					sizeof(struct cam_ife_csid_qcfa_update_args));
				if (rc)
...
static int
cam_isp_blob_core_cfg_update(uint32_t blob_type,
			     struct cam_isp_generic_blob_info *blob_info,
			  ...
			  struct cam_hw_prepare_update_args *prepare)
{
	...
	CAM_DBG(CAM_PERF, "usage=%u left_clk= %lu right_clk=%lu",
	...
	list_for_each_entry (hw_mgr_res, &ctx->res_list_ife_src, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_mgr_res->res_id == CAM_ISP_HW_VFE_IN_CAMIF) {
				...
				for (j = 0; j < clock_config->num_rdi; j++)
					clk_rate = max(clock_config->rdi_hz[j],
						       clk_rate);
			else if (hw_mgr_res->res_id != CAM_ISP_HW_VFE_IN_LCR &&
				 hw_mgr_res->hw_res[i]) {
				CAM_ERR(CAM_ISP, "Invalid res_id %u",
					hw_mgr_res->res_id);
				rc = -EINVAL;
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_DBG(CAM_PERF, "res_id=%u i= %d clk=%llu\n",
...
static int
cam_isp_blob_vfe_out_update(uint32_t blob_type,
			    struct cam_isp_generic_blob_info *blob_info,
			    ...
			    struct cam_hw_prepare_update_args *prepare)
{
	...
	for (i = 0; i < vfe_out_config->num_ports; i++) {
		...
		CAM_DBG(CAM_ISP, "VFE out config idx: %d port: 0x%x", i,
			wm_config->port_type);
		...
		rc = cam_isp_add_cmd_buf_update(
			ife_out_res, blob_type, blob_type_hw_cmd_map[blob_type],
			blob_info->base_info->idx, (void *)cmd_buf_addr,
			kmd_buf_remain_size, (void *)wm_config, &bytes_used);
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
					       ...
					       uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_HFR_CONFIG: {
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu",
				...
				sizeof(struct cam_isp_resource_hfr_config) +
					(hfr_config->num_ports -
					 1) * sizeof(struct cam_isp_port_hfr_config));
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_CLOCK_CONFIG: {
		...
		if (clock_config->num_rdi > 1) {
			if (sizeof(uint64_t) >
			    ((UINT_MAX - sizeof(struct cam_isp_clock_config)) /
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG: {
		...
		if (bw_config->num_rdi > 1) {
			if (sizeof(struct cam_isp_bw_vote) >
			    ((UINT_MAX - sizeof(struct cam_isp_bw_config)) /
			     (bw_config->num_rdi - 1))) {
				CAM_ERR(CAM_ISP,
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V2: {
		...
		if (bw_config->num_paths > 1) {
			if (sizeof(struct cam_axi_per_path_bw_vote) >
			    ((UINT_MAX - sizeof(struct cam_isp_bw_config_v2)) /
		...
		memset(&prepare_hw_data->bw_config_v2[bw_config->usage_type], 0,
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_UBWC_CONFIG: {
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP, "Invalid blob_size %u expected %lu",
				...
				sizeof(struct cam_ubwc_config) +
					(ubwc_config->num_ports -
					 1) * sizeof(struct cam_ubwc_plane_cfg_v1) *
						2);
			return -EINVAL;
		}

		rc = cam_isp_blob_ubwc_update(blob_type, blob_info, ubwc_config,
					      prepare);
		if (rc)
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_UBWC_CONFIG_V2: {
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP, "Invalid blob_size %u expected %lu",
				...
				sizeof(struct cam_ubwc_config_v2) +
					(ubwc_config->num_ports -
					 1) * sizeof(struct cam_ubwc_plane_cfg_v2) *
						2);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_FE_CONFIG: {
		...
		rc = cam_isp_blob_fe_update(blob_type, blob_info, fe_config,
					    prepare);
		if (rc)
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_VFE_OUT_CONFIG: {
		...
		if (vfe_out_config->num_ports != 1) {
			if (sizeof(struct cam_isp_vfe_wm_config) >
			    ((UINT_MAX - sizeof(struct cam_isp_vfe_out_config)) /
		...
		if (blob_size < (sizeof(struct cam_isp_vfe_out_config) +
				 (vfe_out_config->num_ports -
				  1) * sizeof(struct cam_isp_vfe_wm_config))) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu",
				blob_size,
				sizeof(struct cam_isp_vfe_out_config) +
					(vfe_out_config->num_ports -
					 ...
					 void *prepare_hw_update_args)
{
	...
	CAM_DBG(CAM_REQ, "ctx[%pK][%d] Enter for req_id %lld", ctx,
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		rc = cam_isp_add_io_buffers(
			...
			&ctx->res_list_ife_in_rd, CAM_IFE_HW_OUT_RES_MAX,
			fill_fence);
...
static int cam_ife_mgr_sof_irq_debug(struct cam_ife_hw_mgr_ctx *ctx,
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	struct cam_ife_hw_mgr *hw_mgr = hw_mgr_priv;
	struct cam_ife_hw_mgr_ctx *ctx =
		(struct cam_ife_hw_mgr_ctx *)hw_cmd_args->ctxt_to_hw_map;
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		...
		case CAM_ISP_HW_MGR_CMD_SOF_DEBUG:
			cam_ife_mgr_sof_irq_debug(
				ctx, isp_hw_cmd_args->u.sof_irq_enable);
			break;
	...
	case CAM_HW_MGR_CMD_REG_DUMP_ON_FLUSH:
		...
		rc = cam_ife_mgr_handle_reg_dump(
			ctx, ctx->reg_dump_buf_desc, ctx->num_reg_dump_buf,
					     ...
					     uint64_t *boot_time_stamp)
{
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		...
		if (hw_intf->hw_ops.process_cmd) {
			...
			rc = hw_intf->hw_ops.process_cmd(
				hw_intf->hw_priv,
				CAM_IFE_CSID_CMD_GET_TIME_STAMP, &csid_get_time,
				sizeof(struct cam_csid_get_time_stamp_args));
			if (!rc && (i == CAM_ISP_HW_SPLIT_LEFT)) {
				*time_stamp = csid_get_time.time_stamp_val;
				*boot_time_stamp = csid_get_time.boot_timestamp;
			}
		}
	}
...
static int cam_ife_hw_mgr_handle_hw_err(void *evt_info)
{
	...
	rc = cam_ife_hw_mgr_find_affected_ctx(&error_event_data, core_idx,
					      &recovery_data);
...
static int cam_ife_hw_mgr_handle_hw_rup(void *ctx, void *evt_info)
{
	struct cam_isp_hw_event_info *event_info = evt_info;
...
static int cam_ife_hw_mgr_handle_hw_epoch(void *ctx, void *evt_info)
{
	struct cam_isp_hw_event_info *event_info = evt_info;
	struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx = ctx;
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		...
		rc = cam_ife_hw_mgr_check_irq_for_dual_vfe(
			ife_hw_mgr_ctx, CAM_ISP_HW_EVENT_EPOCH);
		if (!rc) {
			if (atomic_read(&ife_hw_mgr_ctx->overflow_pending))
				break;
...
static int cam_ife_hw_mgr_handle_hw_sof(void *ctx, void *evt_info)
{
	struct cam_isp_hw_event_info *event_info = evt_info;
	struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx = ctx;
	...
	case CAM_ISP_HW_VFE_IN_RD:
		...
		rc = cam_ife_hw_mgr_check_irq_for_dual_vfe(
			ife_hw_mgr_ctx, CAM_ISP_HW_EVENT_SOF);
		if (!rc) {
...
static int cam_ife_hw_mgr_handle_hw_eof(void *ctx, void *evt_info)
{
	struct cam_isp_hw_event_info *event_info = evt_info;
	struct cam_ife_hw_mgr_ctx *ife_hw_mgr_ctx = ctx;
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		...
		rc = cam_ife_hw_mgr_check_irq_for_dual_vfe(
			ife_hw_mgr_ctx, CAM_ISP_HW_EVENT_EOF);
		if (!rc) {
			if (atomic_read(&ife_hw_mgr_ctx->overflow_pending))
				break;
...
static int cam_ife_hw_mgr_handle_hw_buf_done(void *ctx, void *evt_info)
{
...
static int cam_ife_set_camif_debug(void *data, u64 val)
{
	...
	CAM_DBG(CAM_ISP, "Set camif enable_diag_sensor_status value :%lld",
		val);
	return 0;
}
...
static int cam_ife_hw_mgr_debug_register(void)
{
	...
	if (!debugfs_create_bool("enable_req_dump", 0644,
				 g_ife_hw_mgr.debug_cfg.dentry,
	...
	if (!debugfs_create_file("ife_camif_debug", 0644,
				 g_ife_hw_mgr.debug_cfg.dentry, NULL,
				 &cam_ife_camif_debug)) {
	...
	if (!debugfs_create_bool("per_req_reg_dump", 0644,
				 g_ife_hw_mgr.debug_cfg.dentry,
