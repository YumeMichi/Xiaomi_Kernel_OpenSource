...
int cam_isp_add_change_base(struct cam_hw_prepare_update_args *prepare,
			    ...
			    struct cam_kmd_buf_info *kmd_buf_info)
{
	...
	if (num_ent + 1 >= prepare->max_hw_update_entries) {
		CAM_ERR(CAM_ISP, "Insufficient  HW entries :%d %d", num_ent,
	...
	list_for_each_entry (hw_mgr_res, res_list_isp_src, list) {
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			get_base.cmd.cmd_buf_addr =
				kmd_buf_info->cpu_addr +
			...
			CAM_DBG(CAM_ISP,
				...
				num_ent, hw_entry[num_ent].handle,
...
static int cam_isp_update_dual_config(
	...
	struct cam_cmd_buf_desc *cmd_desc, uint32_t split_id, uint32_t base_idx,
	struct cam_ife_hw_mgr_res *res_list_isp_out, uint32_t size_isp_out)
{
	int rc = -EINVAL;
	...
	CAM_DBG(CAM_ISP, "cmd des size %d, length: %d", cmd_desc->size,
		cmd_desc->length);
	...
	if ((len < sizeof(struct cam_isp_dual_config)) ||
	    (cmd_desc->offset >= (len - sizeof(struct cam_isp_dual_config)))) {
	...
	for (i = 0; i < dual_config->num_ports; i++) {
		if (i >= CAM_ISP_IFE_OUT_RES_MAX) {
			CAM_ERR(CAM_ISP,
				"failed update for i:%d > size_isp_out:%d", i,
				size_isp_out);
			rc = -EINVAL;
			goto end;
		}
...
int cam_isp_add_cmd_buf_update(struct cam_ife_hw_mgr_res *hw_mgr_res,
			       uint32_t cmd_type, uint32_t hw_cmd_type,
			       uint32_t base_idx, uint32_t *cmd_buf_addr,
			       uint32_t kmd_buf_remain_size,
			       void *cmd_update_data, uint32_t *bytes_used)
{
	int rc = 0;
	struct cam_isp_resource_node *res;
	...
	CAM_DBG(CAM_ISP, "cmd_type %u cmd buffer 0x%pK, size %d",
		cmd_update.cmd_type, cmd_update.cmd.cmd_buf_addr,
		cmd_update.cmd.size);
	...
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		...
		rc = res->hw_intf->hw_ops.process_cmd(
			res->hw_intf->hw_priv, cmd_update.cmd_type, &cmd_update,
			sizeof(struct cam_isp_hw_get_cmd_update));

		if (rc) {
			CAM_ERR(CAM_ISP, "get buf cmd error:%d", res->res_id);
			rc = -ENOMEM;
			return rc;
		}
...
int cam_isp_add_command_buffers(struct cam_hw_prepare_update_args *prepare,
				struct cam_kmd_buf_info *kmd_buf_info,
				...
				uint32_t size_isp_out)
{
	...
	for (i = 0; i < prepare->packet->num_cmd_buf; i++) {
		...
		CAM_DBG(CAM_ISP, "meta type: %d, split_id: %d", cmd_meta_data,
			split_id);
		...
		case CAM_ISP_PACKET_META_DUAL_CONFIG:
			rc = cam_isp_update_dual_config(prepare, &cmd_desc[i],
							split_id, base_idx,
							res_list_isp_out,
							size_isp_out);
		...
		case CAM_ISP_PACKET_META_GENERIC_BLOB_COMMON: {
			...
			rc = cam_packet_util_process_generic_cmd_buffer(
				&cmd_desc[i], blob_handler_cb, &blob_info);
			if (rc) {
				CAM_ERR(CAM_ISP,
...
int cam_isp_add_io_buffers(int iommu_hdl, int sec_iommu_hdl,
			   struct cam_hw_prepare_update_args *prepare,
			   uint32_t base_idx,
			   struct cam_kmd_buf_info *kmd_buf_info,
			   struct cam_ife_hw_mgr_res *res_list_isp_out,
			   struct list_head *res_list_ife_in_rd,
			   uint32_t size_isp_out, bool fill_fence)
{
	int rc = 0;
	...
	for (i = 0; i < prepare->packet->num_io_configs; i++) {
		...
		if (io_cfg[i].direction == CAM_BUF_OUTPUT) {
			...
			if (!list_empty(res_list_ife_in_rd)) {
				hw_mgr_res = list_first_entry(
					res_list_ife_in_rd,
					struct cam_ife_hw_mgr_res, list);
			} else {
				CAM_ERR(CAM_ISP, "No IFE in Read resource");
				return -EINVAL;
			}
			in_map_entries = &prepare->in_map_entries[num_in_buf];
			if (fill_fence) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX &&
			...
			for (plane_id = 0; plane_id < CAM_PACKET_MAX_PLANES;
				...
				if (res->process_cmd(
					    res, CAM_ISP_HW_CMD_GET_SECURE_MODE,
					    &mode, sizeof(bool)))
					return -EINVAL;
				...
				rc = cam_mem_get_io_buf(
					io_cfg[i].mem_handle[plane_id], mmu_hdl,
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX &&
			    io_cfg[i].direction == CAM_BUF_INPUT;
		     j++) {
			if (!hw_mgr_res->hw_res[j])
				continue;
	...
	if (io_cfg_used_bytes) {
		...
		CAM_DBG(CAM_ISP, "num_ent=%d handle=0x%x, len=%u, offset=%u",
			num_ent, prepare->hw_update_entries[num_ent].handle,
...
int cam_isp_add_reg_update(struct cam_hw_prepare_update_args *prepare,
			   struct list_head *res_list_isp_src,
			   uint32_t base_idx,
			   struct cam_kmd_buf_info *kmd_buf_info)
{
