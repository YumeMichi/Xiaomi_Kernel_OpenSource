...
static int cam_ife_csid_reset_regs(struct cam_ife_csid_hw *csid_hw,
				   bool reset_hw);
static int cam_ife_csid_is_ipp_ppp_format_supported(uint32_t in_format)
{
	int rc = -EINVAL;
...
static int cam_ife_csid_get_format_rdi(uint32_t in_format, uint32_t out_format,
				       uint32_t *decode_fmt,
				       uint32_t *plain_fmt,
				       uint32_t *packing_fmt, bool rpp)
{
	int rc = 0;
...
static int cam_ife_csid_get_format_ipp_ppp(uint32_t in_format,
					   uint32_t *decode_fmt,
					   uint32_t *plain_fmt)
{
	int rc = 0;

	CAM_DBG(CAM_ISP, "input format:%d", in_format);
	...
	case CAM_FORMAT_DPCM_12_10_12:
		...
		*plain_fmt = 0x1;
		break;
	default:
		CAM_ERR(CAM_ISP, "Unsupported format %d", in_format);
		rc = -EINVAL;
	}

	CAM_DBG(CAM_ISP, "decode_fmt:%d plain_fmt:%d", *decode_fmt, *plain_fmt);
				    ...
				    struct cam_ife_csid_cid_data *cid_data)
{
	...
	case 2:
		if (vc[1] != cid_data->vc1 || dt[1] != cid_data->dt1)
			return -EINVAL;
	case 1:
		if (vc[0] != cid_data->vc || dt[0] != cid_data->dt)
			return -EINVAL;
		break;
	default:
		return -EINVAL;
...
static int cam_ife_csid_cid_get(struct cam_ife_csid_hw *csid_hw,
				struct cam_isp_resource_node **res, int32_t *vc,
				uint32_t *dt, uint32_t num_valid_vc_dt)
{
	struct cam_ife_csid_cid_data *cid_data;
	uint32_t i = 0;
	...
	*res = NULL;
	...
	for (i = 0; i < CAM_IFE_CSID_CID_MAX; i++) {
		if (csid_hw->cid_res[i].res_state >=
			...
			cid_data = (struct cam_ife_csid_cid_data *)csid_hw
					   ->cid_res[i]
					   .res_priv;
			if (!cam_ife_match_vc_dt_pair(vc, dt, num_valid_vc_dt,
						      cid_data)) {
				cid_data->cnt++;
...
static int cam_ife_csid_global_reset(struct cam_ife_csid_hw *csid_hw)
{
	...
	if (csid_hw->hw_info->hw_state != CAM_HW_STATE_POWER_UP) {
		CAM_ERR(CAM_ISP, "CSID:%d Invalid HW State:%d",
			csid_hw->hw_intf->hw_idx, csid_hw->hw_info->hw_state);
		return -EINVAL;
	}
	...
	cam_io_w_mb(0x80, soc_info->reg_map[0].mem_base +
				  csid_hw->csid_info->csid_reg->csi2_reg
					  ->csid_csi2_rx_cfg1_addr);
				   ...
				   struct cam_csid_reset_cfg_args *reset)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d resource:%d", csid_hw->hw_intf->hw_idx,
		res->res_id);
	...
	if (res->res_id == CAM_IFE_PIX_PATH_RES_IPP) {
		...
		complete = &csid_hw->csid_rdin_complete[id];
		...
		complete = &csid_hw->csid_udin_complete[id];
	...
	cam_io_w_mb(reset_strb_val,
		    soc_info->reg_map[0].mem_base + reset_strb_addr);
	...
	struct cam_csid_hw_reserve_resource_args *cid_reserv)
{
	...
	CAM_DBG(CAM_ISP,
		...
		csid_hw->hw_intf->hw_idx, cid_reserv->in_port->res_type,
		cid_reserv->in_port->lane_type, cid_reserv->in_port->lane_num,
		cid_reserv->in_port->dt[0], cid_reserv->in_port->vc[0]);
	...
	if (cid_reserv->in_port->res_type == CAM_ISP_IFE_IN_RES_TPG &&
		...
		CAM_ERR(CAM_ISP, " CSID:%d Invalid tpg decode fmt %d",
			csid_hw->hw_intf->hw_idx, cid_reserv->in_port->format);
		rc = -EINVAL;
		goto end;
	}

	if (csid_hw->csi2_reserve_cnt == UINT_MAX) {
		CAM_ERR(CAM_ISP, "CSID%d reserve cnt reached max",
			csid_hw->hw_intf->hw_idx);
		rc = -EINVAL;
		goto end;
	}
	...
	case CAM_IFE_PIX_PATH_RES_IPP:
		if (csid_hw->ipp_res.res_state !=
		    CAM_ISP_RESOURCE_STATE_AVAILABLE) {
			CAM_DBG(CAM_ISP, "CSID:%d IPP resource not available",
				csid_hw->hw_intf->hw_idx);
			rc = -EINVAL;
			goto end;
		}
	...
	case CAM_IFE_PIX_PATH_RES_RDI_3:
		if (csid_hw->rdi_res[cid_reserv->res_id].res_state !=
			...
			CAM_ERR(CAM_ISP,
				...
				csid_hw->hw_intf->hw_idx, cid_reserv->res_id);
			rc = -EINVAL;
			goto end;
		}
		break;
	...
	cid_data =
		(struct cam_ife_csid_cid_data *)cid_reserv->node_res->res_priv;

	if (!csid_hw->csi2_reserve_cnt) {
		csid_hw->res_type = cid_reserv->in_port->res_type;

		csid_hw->csi2_rx_cfg.lane_cfg = cid_reserv->in_port->lane_cfg;
		csid_hw->csi2_rx_cfg.lane_type = cid_reserv->in_port->lane_type;
		csid_hw->csi2_rx_cfg.lane_num = cid_reserv->in_port->lane_num;
	...
	CAM_DBG(CAM_ISP, "CSID:%d CID:%d acquired", csid_hw->hw_intf->hw_idx,
			      ...
			      struct cam_csid_hw_reserve_resource_args *reserve)
{
	...
	if (reserve->sync_mode >= CAM_ISP_HW_SYNC_MAX) {
		CAM_ERR(CAM_ISP, "CSID: %d Sync Mode: %d", reserve->sync_mode);
		return -EINVAL;
	}
	...
	for (i = 0; i < reserve->in_port->num_valid_vc_dt; i++) {
		if (reserve->in_port->dt[i] > 0x3f ||
			...
			CAM_ERR(CAM_ISP, "CSID:%d Invalid vc:%d dt %d",
				csid_hw->hw_intf->hw_idx, reserve->in_port->vc,
				reserve->in_port->dt);
			rc = -EINVAL;
			goto end;
		}
	...
	CAM_DBG(CAM_ISP, "CSID:%d res:%d width %d height %d",
		csid_hw->hw_intf->hw_idx, reserve->res_id, path_data->width,
		path_data->height);
...
static int cam_ife_csid_enable_hw(struct cam_ife_csid_hw *csid_hw)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d init CSID HW", csid_hw->hw_intf->hw_idx);
...
static int cam_ife_csid_disable_hw(struct cam_ife_csid_hw *csid_hw)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d De-init CSID HW", csid_hw->hw_intf->hw_idx);
				  ...
				  struct cam_isp_resource_node *res)
{
	...
	if (csid_hw->tpg_start_cnt == 1) {
		...
		soc_info = &csid_hw->hw_info->soc_info;
		{
			...
			for (i = 0; i < 16; i++) {
				val = cam_io_r_mb(
					soc_info->reg_map[0].mem_base + base +
					i * 4);
		...
		if (csid_hw->tpg_cfg.usage_type) {
			rc = cam_ife_csid_enable_ife_force_clock_on(
				soc_info,
				 ...
				 struct cam_isp_resource_node *res)
{
	...
	if (!csid_hw->tpg_start_cnt) {
		...
		if (csid_hw->tpg_cfg.usage_type)
			rc = cam_ife_csid_disable_ife_force_clock_on(
				soc_info,
				csid_reg->tpg_reg->tpg_cpas_ife_reg_offset);
				   ...
				   struct cam_isp_resource_node *res)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d TPG config", csid_hw->hw_intf->hw_idx);
	...
	cam_io_w_mb(0x12345678, soc_info->reg_map[0].mem_base +
					csid_hw->csid_info->csid_reg->tpg_reg
						->csid_tpg_lfsr_seed_addr);

	val = csid_hw->tpg_cfg.width << 16 | csid_hw->tpg_cfg.height;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				 csid_reg->tpg_reg->csid_tpg_dt_n_cfg_0_addr);

	cam_io_w_mb(CAM_IFE_CSID_TPG_DT_VAL,
		    soc_info->reg_map[0].mem_base +
...
static int cam_ife_csid_enable_csi2(struct cam_ife_csid_hw *csid_hw,
				    struct cam_isp_resource_node *res)
{
	int rc = 0;
	const struct cam_ife_csid_reg_offset *csid_reg;
	...
	val |= (csid_hw->csi2_rx_cfg.phy_sel &
		csid_reg->csi2_reg->csi2_rx_phy_num_mask)
	       << 20;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
	...
	if (cid_data->vc > 3 ||
	    (cid_data->is_valid_vc1_dt1 && cid_data->vc1 > 3))
	...
	val = CSID_CSI2_RX_INFO_RST_DONE | CSID_CSI2_RX_ERROR_TG_FIFO_OVERFLOW |
	      ...
	      CSID_CSI2_RX_ERROR_CPHY_SOT_RECEPTION | CSID_CSI2_RX_ERROR_CRC |
	      CSID_CSI2_RX_ERROR_ECC | CSID_CSI2_RX_ERROR_MMAPPED_VC_DT |
...
static int cam_ife_csid_disable_csi2(struct cam_ife_csid_hw *csid_hw,
				     struct cam_isp_resource_node *res)
{
	const struct cam_ife_csid_reg_offset *csid_reg;
	struct cam_hw_soc_info *soc_info;
...
static void cam_ife_csid_halt_csi2(struct cam_ife_csid_hw *csid_hw)
{
	const struct cam_ife_csid_reg_offset *csid_reg;
	struct cam_hw_soc_info *soc_info;
...
static int cam_ife_csid_init_config_pxl_path(struct cam_ife_csid_hw *csid_hw,
					     struct cam_isp_resource_node *res)
{
	int rc = 0;
	struct cam_ife_csid_path_cfg *path_data;
	...
	if (is_ipp && csid_hw->binning_supported && csid_hw->binning_enable)
		val |= (1 << pxl_reg->quad_cfa_bin_en_shift_val);
	...
	if (path_data->is_valid_vc1_dt1 &&
		...
		val |= ((path_data->vc1 << 2) | (path_data->dt1 << 7) | 1);
	}
...
static int cam_ife_csid_deinit_pxl_path(struct cam_ife_csid_hw *csid_hw,
					struct cam_isp_resource_node *res)
{
	int rc = 0;
	uint32_t val;
...
static int cam_ife_csid_enable_pxl_path(struct cam_ife_csid_hw *csid_hw,
					struct cam_isp_resource_node *res)
{
	const struct cam_ife_csid_reg_offset *csid_reg;
	struct cam_hw_soc_info *soc_info;
	...
	CAM_DBG(CAM_ISP, "CSID:%d %s Ctrl val: 0x%x", csid_hw->hw_intf->hw_idx,
		(is_ipp) ? "IPP" : "PPP", val);
	...
	CAM_DBG(CAM_ISP, "Enable %s IRQ mask 0x%x", (is_ipp) ? "IPP" : "PPP",
		val);
...
static int cam_ife_csid_disable_pxl_path(struct cam_ife_csid_hw *csid_hw,
					 struct cam_isp_resource_node *res,
					 enum cam_ife_csid_halt_cmd stop_cmd)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d res_id:%d %s path", csid_hw->hw_intf->hw_idx,
		res->res_id, (is_ipp) ? "IPP" : "PPP");
...
static int cam_ife_csid_init_config_rdi_path(struct cam_ife_csid_hw *csid_hw,
					     struct cam_isp_resource_node *res)
{
	int rc = 0;
	struct cam_ife_csid_path_cfg *path_data;
	...
	/* select the post irq sub sample strobe for time stamp capture */
	cam_io_w_mb(CSID_TIMESTAMP_STB_POST_IRQ,
		    soc_info->reg_map[0].mem_base +
	...
	if (csid_reg->rdi_reg[id]->overflow_ctrl_en) {
		...
		cam_io_w_mb(val,
			    soc_info->reg_map[0].mem_base +
				    csid_reg->rdi_reg[id]
					    ->csid_rdi_err_recovery_cfg0_addr);
	}
	...
	cam_io_w_mb(1, soc_info->reg_map[0].mem_base +
			       csid_reg->rdi_reg[id]
				       ->csid_rdi_irq_subsample_period_addr);
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
			       csid_reg->rdi_reg[id]
				       ->csid_rdi_irq_subsample_pattern_addr);

	/* set pixel drop pattern to 0 and period to 1 */
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
			       csid_reg->rdi_reg[id]
				       ->csid_rdi_rpp_pix_drop_pattern_addr);
	...
	if (camera_hw_version == CAM_CPAS_TITAN_480_V100 &&
		...
		cam_io_w_mb(
			...
			soc_info->reg_map[0].mem_base +
				csid_reg->rdi_reg[id]
					->csid_rdi_rpp_pix_drop_period_addr);
	else
		cam_io_w_mb(
			1, soc_info->reg_map[0].mem_base +
				   csid_reg->rdi_reg[id]
					   ->csid_rdi_rpp_pix_drop_period_addr);
	...
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
			       csid_reg->rdi_reg[id]
				       ->csid_rdi_rpp_line_drop_pattern_addr);
	cam_io_w_mb(1, soc_info->reg_map[0].mem_base +
			       csid_reg->rdi_reg[id]
					     ...
					     struct cam_isp_resource_node *res)
{
	...
	if (csid_reg->udi_reg[id]->overflow_ctrl_en) {
		...
		cam_io_w_mb(val,
			    soc_info->reg_map[0].mem_base +
				    csid_reg->udi_reg[id]
					    ->csid_udi_err_recovery_cfg0_addr);
	}
	...
	cam_io_w_mb(1, soc_info->reg_map[0].mem_base +
			       csid_reg->udi_reg[id]
				       ->csid_udi_irq_subsample_period_addr);
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
			       csid_reg->udi_reg[id]
				       ->csid_udi_irq_subsample_pattern_addr);

	/* set pixel drop pattern to 0 and period to 1 */
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
			       csid_reg->udi_reg[id]
				       ->csid_udi_rpp_pix_drop_pattern_addr);
	cam_io_w_mb(1, soc_info->reg_map[0].mem_base +
			       csid_reg->udi_reg[id]
				       ->csid_udi_rpp_pix_drop_period_addr);
	/* set line drop pattern to 0 and period to 1 */
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
			       csid_reg->udi_reg[id]
				       ->csid_udi_rpp_line_drop_pattern_addr);
	cam_io_w_mb(1, soc_info->reg_map[0].mem_base +
			       csid_reg->udi_reg[id]
				       ->csid_udi_rpp_line_drop_period_addr);
...
static int cam_ife_csid_deinit_rdi_path(struct cam_ife_csid_hw *csid_hw,
					struct cam_isp_resource_node *res)
{
	int rc = 0;
	uint32_t id, val, format_measure_addr;
...
static int cam_ife_csid_enable_rdi_path(struct cam_ife_csid_hw *csid_hw,
					struct cam_isp_resource_node *res)
{
	const struct cam_ife_csid_reg_offset *csid_reg;
	struct cam_hw_soc_info *soc_info;
...
static int cam_ife_csid_disable_rdi_path(struct cam_ife_csid_hw *csid_hw,
					 struct cam_isp_resource_node *res,
					 enum cam_ife_csid_halt_cmd stop_cmd)
{
	int rc = 0;
	...
	CAM_DBG(CAM_ISP, "CSID:%d res_id:%d", csid_hw->hw_intf->hw_idx,
		res->res_id);
...
static int cam_ife_csid_poll_stop_status(struct cam_ife_csid_hw *csid_hw,
					 uint32_t res_mask)
{
	int rc = 0, id;
	...
	for (; res_id < CAM_IFE_PIX_PATH_RES_MAX; res_id++, res_mask >>= 1) {
		...
		rc = readl_poll_timeout(
			...
			(val & 0x1) == 0x1, CAM_IFE_CSID_TIMEOUT_SLEEP_US,
...
static int cam_ife_csid_get_hbi_vbi(struct cam_ife_csid_hw *csid_hw,
				    struct cam_isp_resource_node *res)
{
	uint32_t hbi, vbi;
	int32_t id;
	...
	CAM_INFO_RATE_LIMIT(
		CAM_ISP, "Device %s index %u Resource %u HBI: 0x%x VBI: 0x%x",
		soc_info->dev_name, soc_info->index, res->res_id, hbi, vbi);

	return 0;
}

static int cam_ife_csid_get_time_stamp(struct cam_ife_csid_hw *csid_hw,
				       void *cmd_args)
{
	...
	time_stamp->time_stamp_val =
		mul_u64_u32_div(time_stamp->time_stamp_val,
	...
	if (!csid_hw->prev_boot_timestamp) {
		...
		CAM_DBG(CAM_ISP, "timestamp:%lld", time_stamp->boot_timestamp);
	} else {
		time_delta =
			time_stamp->time_stamp_val - csid_hw->prev_qtimer_ts;
		time_stamp->boot_timestamp =
...
int cam_ife_csid_get_hw_caps(void *hw_priv, void *get_hw_cap_args,
			     uint32_t arg_size)
{
	int rc = 0;
	...
	CAM_DBG(CAM_ISP,
		...
		csid_hw->hw_intf->hw_idx, hw_caps->num_rdis, hw_caps->num_pix,
		hw_caps->major_version, hw_caps->minor_version,
		hw_caps->version_incr);

	return rc;
}

int cam_ife_csid_reset(void *hw_priv, void *reset_args, uint32_t arg_size)
{
	struct cam_ife_csid_hw *csid_hw;
	struct cam_hw_info *csid_hw_info;
	struct cam_csid_reset_cfg_args *reset;
	...
	if (!hw_priv || !reset_args ||
	    (arg_size != sizeof(struct cam_csid_reset_cfg_args))) {
		CAM_ERR(CAM_ISP, "CSID:Invalid args");
		return -EINVAL;
	}
...
int cam_ife_csid_reserve(void *hw_priv, void *reserve_args, uint32_t arg_size)
{
	int rc = 0;
	struct cam_ife_csid_hw *csid_hw;
	struct cam_hw_info *csid_hw_info;
	...
	if (!hw_priv || !reserve_args ||
	    (arg_size != sizeof(struct cam_csid_hw_reserve_resource_args))) {
		CAM_ERR(CAM_ISP, "CSID: Invalid args");
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "res_type %d, CSID: %u", reserv->res_type,
		csid_hw->hw_intf->hw_idx);
...
int cam_ife_csid_release(void *hw_priv, void *release_args, uint32_t arg_size)
{
	int rc = 0;
	struct cam_ife_csid_hw *csid_hw;
	struct cam_hw_info *csid_hw_info;
	...
	if ((res->res_state <= CAM_ISP_RESOURCE_STATE_AVAILABLE) ||
	    (res->res_state >= CAM_ISP_RESOURCE_STATE_STREAMING)) {
		CAM_WARN(CAM_ISP, "CSID:%d res type:%d Res %d in state %d",
			 csid_hw->hw_intf->hw_idx, res->res_type, res->res_id,
			 res->res_state);
		goto end;
	}
	...
	case CAM_ISP_RESOURCE_CID:
		...
		CAM_DBG(CAM_ISP, "CSID:%d res id :%d cnt:%d reserv cnt:%d",
			csid_hw->hw_intf->hw_idx, res->res_id, cid_data->cnt,
			csid_hw->csi2_reserve_cnt);
...
int cam_ife_csid_init_hw(void *hw_priv, void *init_args, uint32_t arg_size)
{
	int rc = 0;
	struct cam_ife_csid_hw *csid_hw;
	struct cam_hw_info *csid_hw_info;
...
int cam_ife_csid_deinit_hw(void *hw_priv, void *deinit_args, uint32_t arg_size)
{
	int rc = 0;
	struct cam_ife_csid_hw *csid_hw;
	struct cam_hw_info *csid_hw_info;
...
int cam_ife_csid_start(void *hw_priv, void *start_args, uint32_t arg_size)
{
	int rc = 0;
	struct cam_ife_csid_hw *csid_hw;
	struct cam_hw_info *csid_hw_info;
...
int cam_ife_csid_stop(void *hw_priv, void *stop_args, uint32_t arg_size)
{
	...
	for (i = 0; i < csid_stop->num_res; i++) {
		...
		case CAM_ISP_RESOURCE_PIX_PATH:
			...
			if (res->res_id == CAM_IFE_PIX_PATH_RES_IPP ||
			    res->res_id == CAM_IFE_PIX_PATH_RES_PPP) {
				rc = cam_ife_csid_disable_pxl_path(
					csid_hw, res, csid_stop->stop_cmd);
			} else if (res->res_id == CAM_IFE_PIX_PATH_RES_RDI_0 ||
				   res->res_id == CAM_IFE_PIX_PATH_RES_RDI_1 ||
				   res->res_id == CAM_IFE_PIX_PATH_RES_RDI_2 ||
				   res->res_id == CAM_IFE_PIX_PATH_RES_RDI_3) {
				rc = cam_ife_csid_disable_rdi_path(
					csid_hw, res, csid_stop->stop_cmd);
			} else if (res->res_id == CAM_IFE_PIX_PATH_RES_UDI_0 ||
				   res->res_id == CAM_IFE_PIX_PATH_RES_UDI_1 ||
				   res->res_id == CAM_IFE_PIX_PATH_RES_UDI_2) {
				rc = cam_ife_csid_disable_udi_path(
					csid_hw, res, csid_stop->stop_cmd);
			} else {
...
int cam_ife_csid_read(void *hw_priv, void *read_args, uint32_t arg_size)
{
...
int cam_ife_csid_write(void *hw_priv, void *write_args, uint32_t arg_size)
{
	CAM_ERR(CAM_ISP, "CSID: un supported");
	return -EINVAL;
}

static int cam_ife_csid_sof_irq_debug(struct cam_ife_csid_hw *csid_hw,
				      void *cmd_args)
{
	int i = 0;
	uint32_t val = 0;
	...
	if (csid_hw->hw_info->hw_state == CAM_HW_STATE_POWER_DOWN) {
		CAM_WARN(CAM_ISP, "CSID powered down unable to %s sof irq",
...
static int cam_ife_csid_set_csid_clock(struct cam_ife_csid_hw *csid_hw,
				       void *cmd_args)
{
	...
	clk_update = (struct cam_ife_csid_clock_update_args *)cmd_args;
...
static int cam_ife_csid_set_csid_qcfa(struct cam_ife_csid_hw *csid_hw,
				      void *cmd_args)
{
	...
	qcfa_update = (struct cam_ife_csid_qcfa_update_args *)cmd_args;
...
static int cam_ife_csid_process_cmd(void *hw_priv, uint32_t cmd_type,
				    void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_IFE_CSID_CMD_GET_TIME_STAMP:
		...
		if (csid_hw->csid_debug & CSID_DEBUG_ENABLE_HBI_VBI_INFO) {
			res = ((struct cam_csid_get_time_stamp_args *)cmd_args)
				      ->node_res;
...
irqreturn_t cam_ife_csid_irq(int irq_num, void *data)
{
	...
	spin_lock_irqsave(&csid_hw->hw_info->hw_lock, flags);
	/* clear */
	cam_io_w_mb(irq_status_top,
		    soc_info->reg_map[0].mem_base +
			    csid_reg->cmn_reg->csid_top_irq_clear_addr);

	cam_io_w_mb(irq_status_rx,
		    soc_info->reg_map[0].mem_base +
			    csid_reg->csi2_reg->csid_csi2_rx_irq_clear_addr);
	if (csid_reg->cmn_reg->num_pix)
		cam_io_w_mb(irq_status_ipp,
			    soc_info->reg_map[0].mem_base +
				    csid_reg->ipp_reg->csid_pxl_irq_clear_addr);

	if (csid_reg->cmn_reg->num_ppp)
		cam_io_w_mb(irq_status_ppp,
			    soc_info->reg_map[0].mem_base +
				    csid_reg->ppp_reg->csid_pxl_irq_clear_addr);
	...
	if (csid_hw->error_irq_count > CAM_IFE_CSID_MAX_IRQ_ERROR_COUNT) {
		fatal_err_detected = true;
		csid_hw->error_irq_count = 0;
	}
	...
	if ((csid_hw->csid_debug & CSID_DEBUG_ENABLE_LONG_PKT_CAPTURE) &&
		...
		CAM_INFO_RATE_LIMIT(
			CAM_ISP, "CSID:%d long pkt cal CRC:%d expected CRC:%d",
	...
	for (i = 0; i < csid_reg->cmn_reg->num_rdis; i++) {
		...
		if ((irq_status_rdi[i] & CSID_PATH_INFO_INPUT_SOF) &&
		    (csid_hw->csid_debug & CSID_DEBUG_ENABLE_SOF_IRQ)) {
			CAM_INFO_RATE_LIMIT(CAM_ISP, "CSID RDI:%d SOF received",
					    i);
			if (csid_hw->sof_irq_triggered)
				csid_hw->irq_debug_cnt++;
		}
		...
		if ((irq_status_rdi[i] & CSID_PATH_INFO_INPUT_EOF) &&
		    (csid_hw->csid_debug & CSID_DEBUG_ENABLE_EOF_IRQ))
			CAM_INFO_RATE_LIMIT(CAM_ISP, "CSID RDI:%d EOF received",
					    i);
	...
	for (i = 0; i < csid_reg->cmn_reg->num_udis; i++) {
		...
		if ((irq_status_udi[i] & CSID_PATH_INFO_INPUT_SOF) &&
		    (csid_hw->csid_debug & CSID_DEBUG_ENABLE_SOF_IRQ)) {
			CAM_INFO_RATE_LIMIT(CAM_ISP, "CSID UDI:%d SOF received",
					    i);
			if (csid_hw->sof_irq_triggered)
				csid_hw->irq_debug_cnt++;
		}
		...
		if ((irq_status_udi[i] & CSID_PATH_INFO_INPUT_EOF) &&
		    (csid_hw->csid_debug & CSID_DEBUG_ENABLE_EOF_IRQ))
			CAM_INFO_RATE_LIMIT(CAM_ISP, "CSID UDI:%d EOF received",
					    i);
			       ...
			       uint32_t csid_idx, bool is_custom)
{
	...
	CAM_DBG(CAM_ISP, "type %d index %d", ife_csid_hw->hw_intf->hw_type,
		csid_idx);
	...
	if (ife_csid_hw->csid_info->csid_reg->cmn_reg->num_pix) {
		...
		path_data = kzalloc(sizeof(*path_data), GFP_KERNEL);
		if (!path_data) {
			rc = -ENOMEM;
			goto err;
		}
	...
	for (i = 0; i < ife_csid_hw->csid_info->csid_reg->cmn_reg->num_rdis;
		...
		ife_csid_hw->rdi_res[i].res_type = CAM_ISP_RESOURCE_PIX_PATH;
	...
	for (i = 0; i < ife_csid_hw->csid_info->csid_reg->cmn_reg->num_udis;
		...
		ife_csid_hw->udi_res[i].res_type = CAM_ISP_RESOURCE_PIX_PATH;
		ife_csid_hw->udi_res[i].res_id = i + CAM_IFE_PIX_PATH_RES_UDI_0;
...
err:
	if (rc) {
		...
		for (i = 0;
		     i < ife_csid_hw->csid_info->csid_reg->cmn_reg->num_rdis;
		     i++)
			kfree(ife_csid_hw->rdi_res[i].res_priv);

		for (i = 0;
		     i < ife_csid_hw->csid_info->csid_reg->cmn_reg->num_udis;
		     i++)
