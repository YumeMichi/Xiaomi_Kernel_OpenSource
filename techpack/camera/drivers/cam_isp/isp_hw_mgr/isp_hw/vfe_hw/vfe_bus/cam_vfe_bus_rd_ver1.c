...
struct cam_vfe_bus_rd_ver1_priv {
	...
	struct cam_isp_resource_node bus_client[CAM_VFE_BUS_RD_VER1_MAX_CLIENTS];
	struct cam_isp_resource_node
		vfe_bus_rd[CAM_VFE_BUS_RD_VER1_VFE_BUSRD_MAX];
...
static int
cam_vfe_bus_get_num_rm(enum cam_vfe_bus_rd_ver1_vfe_bus_rd_type res_type)
{
	switch (res_type) {
	case CAM_VFE_BUS_RD_VER1_VFE_BUSRD_RDI0:
		return 1;
...
static int cam_vfe_bus_acquire_rm(
	...
	void *ctx, enum cam_vfe_bus_rd_ver1_vfe_bus_rd_type vfe_bus_rd_res_id,
	enum cam_vfe_bus_plane_type plane, uint32_t subscribe_irq,
	struct cam_isp_resource_node **rm_res, uint32_t *client_done_mask,
	uint32_t is_dual)
{
	uint32_t rm_idx = 0;
...
{
	int rc = 0;
	struct cam_vfe_bus_rd_ver1_rm_resource_data *rm_data = rm_res->res_priv;
	struct cam_vfe_bus_rd_ver1_common_data *common_data =
		rm_data->common_data;
	uint32_t buf_size;
	uint32_t val;
...
static int
cam_vfe_bus_init_rm_resource(uint32_t index,
			     struct cam_vfe_bus_rd_ver1_priv *ver1_bus_rd_priv,
...
static int cam_vfe_bus_deinit_rm_resource(struct cam_isp_resource_node *rm_res)
{
	struct cam_vfe_bus_rd_ver1_rm_resource_data *rsrc_data;
					  ...
					  uint32_t args_size)
{
	...
	struct cam_vfe_bus_rd_ver1_priv *ver1_bus_rd_priv = bus_priv;
	...
	CAM_DBG(CAM_ISP, "Acquiring resource type 0x%x", acq_args->rsrc_type);

	bus_rd_res_id = cam_vfe_bus_get_bus_rd_res_id(acq_args->rsrc_type);
	...
	for (i = 0; i < num_rm; i++) {
		rc = cam_vfe_bus_acquire_rm(
			ver1_bus_rd_priv, bus_rd_acquire_args->out_port_info,
			acq_args->tasklet, acq_args->priv, bus_rd_res_id, i,
			subscribe_irq, &rsrc_data->rm_res[i], &client_done_mask,
...
static int cam_vfe_bus_start_vfe_bus_rd(struct cam_isp_resource_node *vfe_out)
{
	int rc = 0, i;
	struct cam_vfe_bus_rd_ver1_vfe_bus_rd_data *rsrc_data = NULL;
...
static int cam_vfe_bus_stop_vfe_bus_rd(struct cam_isp_resource_node *vfe_bus_rd)
{
	int rc = 0, i;
	struct cam_vfe_bus_rd_ver1_vfe_bus_rd_data *rsrc_data = NULL;
...
static int cam_vfe_bus_init_vfe_bus_read_resource(
	uint32_t index, struct cam_vfe_bus_rd_ver1_priv *bus_rd_priv,
	struct cam_vfe_bus_rd_ver1_hw_info *bus_rd_hw_info)
{
	struct cam_isp_resource_node *vfe_bus_rd = NULL;
	struct cam_vfe_bus_rd_ver1_vfe_bus_rd_data *rsrc_data = NULL;
				    ...
				    uint32_t arg_size)
{
	...
	for (i = 0, j = 0; i < vfe_bus_rd_data->num_rm; i++) {
		...
		CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
					 rm_data->hw_regs->buf_size, buf_size);
		CAM_DBG(CAM_ISP, "RM %d image size 0x%x", rm_data->index,
			reg_val_pair[j - 1]);
		...
		rm_data->stride = val;
		CAM_DBG(CAM_ISP, "RM %d image stride 0x%x", rm_data->index,
			reg_val_pair[j - 1]);
		...
		CAM_VFE_ADD_REG_VAL_PAIR(
			reg_val_pair, j, rm_data->hw_regs->image_addr,
			update_buf->rm_update->image_buf[i] + rm_data->offset);
		CAM_DBG(CAM_ISP, "RM %d image address 0x%x", rm_data->index,
			reg_val_pair[j - 1]);
		rm_data->img_addr = reg_val_pair[j - 1];
	}

	size = vfe_bus_rd_data->cdm_util_ops->cdm_required_size_reg_random(j /
									   2);
			       ...
			       uint32_t arg_size)
{
	...
	bus_priv->irq_handle = cam_irq_controller_subscribe_irq(
		...
		top_irq_reg_mask, bus_priv, cam_vfe_bus_rd_ver1_handle_irq,
		NULL, NULL, NULL);
...
int cam_vfe_bus_rd_ver1_init(struct cam_hw_soc_info *soc_info,
			     ...
			     struct cam_vfe_bus **vfe_bus)
{
	...
	bus_priv = kzalloc(sizeof(struct cam_vfe_bus_rd_ver1_priv), GFP_KERNEL);
	if (!bus_priv) {
	...
	bus_priv->num_bus_rd_resc = bus_rd_hw_info->num_bus_rd_resc;
...
int cam_vfe_bus_rd_bus_ver1_deinit(struct cam_vfe_bus **vfe_bus)
{
	...
	for (i = 0; i < bus_priv->num_client; i++) {
		...
		if (rc < 0)
			CAM_ERR(CAM_ISP, "Deinit RM failed rc=%d", rc);
	}
