...
#define MAX_BUF_UPDATE_REG_NUM                                                 \
	((sizeof(struct cam_vfe_bus_ver3_reg_offset_bus_client) +              \
	  sizeof(struct cam_vfe_bus_ver3_reg_offset_ubwc_client)) /            \
	 4)
...
struct cam_vfe_bus_ver3_common_data {
	...
	struct cam_vfe_bus_irq_evt_payload
		evt_payload[CAM_VFE_BUS_VER3_PAYLOAD_MAX];
...
static int cam_vfe_bus_ver3_process_cmd(struct cam_isp_resource_node *priv,
	...
	struct cam_vfe_bus_irq_evt_payload **evt_payload)
{
	...
	*evt_payload =
		list_first_entry(&common_data->free_payload_list,
	...
	uint32_t *intra_client_mask)
{
	...
	*intra_client_mask = 0;
	...
	case CAM_VFE_BUS_VER3_VFE_CORE_1:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid value for master core %u",
			current_core);
		rc = -EINVAL;
		break;
	}
...
static int
cam_vfe_bus_ver3_get_num_wm(enum cam_vfe_bus_ver3_vfe_out_type res_type,
			    uint32_t format)
{
	switch (res_type) {
	case CAM_VFE_BUS_VER3_VFE_OUT_RDI0:
	...
	CAM_ERR(CAM_ISP, "Unsupported format %u for out_type:0x%X", format,
		res_type);
...
static int
cam_vfe_bus_ver3_get_wm_idx(enum cam_vfe_bus_ver3_vfe_out_type vfe_out_res_id,
			    enum cam_vfe_bus_plane_type plane, bool is_lite)
{
	int wm_idx = -1;
...
static int cam_vfe_bus_ver3_get_comp_vfe_out_res_id_list(uint32_t comp_mask,
							 uint32_t *out_list,
							 int *num_out,
							 bool is_lite)
{
...
static int
cam_vfe_bus_ver3_handle_rup_top_half(uint32_t evt_id,
				     struct cam_irq_th_payload *th_payload)
{
	...
	if (rc) {
		CAM_INFO_RATE_LIMIT(CAM_ISP, "VFE:%d Bus IRQ status_0: 0x%X",
				    rsrc_data->common_data->core_index,
				    th_payload->evt_status_arr[0]);
		return rc;
	}
						   ...
						   void *evt_payload_priv)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%d Bus RUP IRQ status_0:0x%X rc:%d",
...
static int cam_vfe_bus_ver3_acquire_wm(
	...
	struct cam_isp_out_port_generic_info *out_port_info, void *tasklet,
	enum cam_vfe_bus_ver3_vfe_out_type vfe_out_res_id,
	enum cam_vfe_bus_plane_type plane,
	struct cam_isp_resource_node **wm_res, uint32_t *client_done_mask,
	uint32_t is_dual, enum cam_vfe_bus_ver3_comp_grp_type *comp_grp_id)
{
	int32_t wm_idx = 0;
	...
	*wm_res = NULL;
	...
	wm_idx = cam_vfe_bus_ver3_get_wm_idx(
		vfe_out_res_id, plane, ver3_bus_priv->common_data.is_lite);
	...
	if (wm_res_local->res_state != CAM_ISP_RESOURCE_STATE_AVAILABLE) {
		CAM_ERR(CAM_ISP, "WM:%d not available state:%d", wm_idx,
	...
	rsrc_data->pack_fmt =
		cam_vfe_bus_ver3_get_packer_fmt(rsrc_data->format, wm_idx);
	...
	} else if ((rsrc_data->index < 2) || (rsrc_data->index == 8) ||
		   (rsrc_data->index == 9) || (rsrc_data->index == 4) ||
		   (rsrc_data->index == 5)) {
		/*
...
static int
cam_vfe_bus_ver3_handle_wm_done_top_half(uint32_t evt_id,
					 struct cam_irq_th_payload *th_payload)
{
	return -EPERM;
}
...
static int
cam_vfe_bus_ver3_init_wm_resource(uint32_t index,
				  struct cam_vfe_bus_ver3_priv *ver3_bus_priv,
...
static int
cam_vfe_bus_ver3_deinit_wm_resource(struct cam_isp_resource_node *wm_res)
{
	struct cam_vfe_bus_ver3_wm_resource_data *rsrc_data;
...
static void
cam_vfe_bus_ver3_add_wm_to_comp_grp(struct cam_isp_resource_node *comp_grp,
				    uint32_t composite_mask)
{
...
static bool
cam_vfe_bus_ver3_match_comp_grp(struct cam_vfe_bus_ver3_priv *ver3_bus_priv,
				struct cam_isp_resource_node **comp_grp,
				uint32_t comp_grp_id)
{
	...
	list_for_each_entry (comp_grp_local, &ver3_bus_priv->used_comp_grp,
			     list) {
	...
	list_for_each_entry (comp_grp_local, &ver3_bus_priv->free_comp_grp,
			     list) {
		rsrc_data = comp_grp_local->res_priv;
		if (rsrc_data->comp_grp_type == comp_grp_id) {
			/* Match found */
...
static int cam_vfe_bus_ver3_acquire_comp_grp(
	struct cam_vfe_bus_ver3_priv *ver3_bus_priv,
	struct cam_isp_out_port_generic_info *out_port_info, void *tasklet,
	uint32_t is_dual, uint32_t is_master,
	enum cam_vfe_bus_ver3_vfe_core_id dual_slave_core,
	struct cam_isp_resource_node **comp_grp,
...
static int
cam_vfe_bus_ver3_release_comp_grp(struct cam_vfe_bus_ver3_priv *ver3_bus_priv,
				  struct cam_isp_resource_node *in_comp_grp)
{
	...
	if (in_comp_grp->res_state == CAM_ISP_RESOURCE_STATE_STREAMING) {
		CAM_ERR(CAM_ISP, "Invalid State %d", in_comp_grp->res_state);
		return -EBUSY;
	}
...
static int
cam_vfe_bus_ver3_start_comp_grp(struct cam_isp_resource_node *comp_grp,
...
static int
cam_vfe_bus_ver3_stop_comp_grp(struct cam_isp_resource_node *comp_grp)
{
...
static int cam_vfe_bus_ver3_handle_comp_done_top_half(
	uint32_t evt_id, struct cam_irq_th_payload *th_payload)
{
	return -EPERM;
}

static int cam_vfe_bus_ver3_handle_comp_done_bottom_half(void *handler_priv,
							 void *evt_payload_priv,
							 uint32_t *comp_mask)
{
...
static int
cam_vfe_bus_ver3_init_comp_grp(uint32_t index, struct cam_hw_soc_info *soc_info,
			       struct cam_vfe_bus_ver3_priv *ver3_bus_priv,
			       struct cam_vfe_bus_ver3_hw_info *ver3_hw_info,
			       struct cam_isp_resource_node *comp_grp)
{
...
static int
cam_vfe_bus_ver3_deinit_comp_grp(struct cam_isp_resource_node *comp_grp)
{
	struct cam_vfe_bus_ver3_comp_grp_data *rsrc_data = comp_grp->res_priv;
					    ...
					    uint32_t arg_size)
{
	...
	*mode = (rsrc_data->secure_mode == CAM_SECURE_MODE_SECURE) ? true :
								     false;
					    ...
					    uint32_t args_size)
{
	...
	for (i = 0; i < num_wm; i++) {
		rc = cam_vfe_bus_ver3_acquire_wm(
			ver3_bus_priv, out_acquire_args->out_port_info,
			acq_args->tasklet, vfe_out_res_id, i,
			&rsrc_data->wm_res[i], &client_done_mask,
			out_acquire_args->is_dual, &comp_grp_id);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	rc = cam_vfe_bus_ver3_acquire_comp_grp(
		ver3_bus_priv, out_acquire_args->out_port_info,
		acq_args->tasklet, out_acquire_args->is_dual,
		out_acquire_args->is_master, out_acquire_args->dual_slave_core,
		&rsrc_data->comp_grp, comp_grp_id);
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Failed to acquire comp_grp VFE:%d out_typp:%d rc:%d",
			rsrc_data->common_data->core_index, vfe_out_res_id, rc);
		return rc;
	}
...
static int cam_vfe_bus_ver3_start_vfe_out(struct cam_isp_resource_node *vfe_out)
{
	int rc = 0, i;
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data = NULL;
	struct cam_vfe_bus_ver3_common_data *common_data = NULL;
	...
	if (!common_data->rup_irq_handle[source_group]) {
		...
		rup_irq_reg_mask[CAM_VFE_BUS_VER3_IRQ_REG0] |= 0x1
							       << source_group;
		...
		common_data->rup_irq_handle[source_group] =
			cam_irq_controller_subscribe_irq(
				...
				CAM_IRQ_PRIORITY_0, rup_irq_reg_mask, vfe_out,
				cam_vfe_bus_ver3_handle_rup_top_half,
				cam_vfe_bus_ver3_handle_rup_bottom_half,
				vfe_out->tasklet_info, &tasklet_bh_api);
...
static int cam_vfe_bus_ver3_handle_vfe_out_done_top_half(
	uint32_t evt_id, struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	int i;
	...
	if (rc) {
		CAM_INFO_RATE_LIMIT(
			CAM_ISP, "VFE:%d Bus IRQ status_0: 0x%X status_1: 0x%X",
			rsrc_data->common_data->core_index,
			th_payload->evt_status_arr[0],
...
static int
cam_vfe_bus_ver3_handle_vfe_out_done_bottom_half(void *handler_priv,
						 void *evt_payload_priv)
{
...
static int cam_vfe_bus_ver3_init_vfe_out_resource(
	uint32_t index, struct cam_vfe_bus_ver3_priv *ver3_bus_priv,
	struct cam_vfe_bus_ver3_hw_info *ver3_hw_info)
{
	struct cam_isp_resource_node *vfe_out = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data = NULL;
	...
	if (vfe_out_type < 0 || vfe_out_type >= CAM_VFE_BUS_VER3_VFE_OUT_MAX) {
	...
	rsrc_data->out_type = ver3_hw_info->vfe_out_hw_info[index].vfe_out_type;
	rsrc_data->common_data = &ver3_bus_priv->common_data;
	rsrc_data->max_width = ver3_hw_info->vfe_out_hw_info[index].max_width;
	rsrc_data->max_height = ver3_hw_info->vfe_out_hw_info[index].max_height;
	rsrc_data->secure_mode = CAM_SECURE_MODE_NON_SECURE;
...
static int
cam_vfe_bus_ver3_deinit_vfe_out_resource(struct cam_isp_resource_node *vfe_out)
{
	struct cam_vfe_bus_ver3_vfe_out_data *rsrc_data = vfe_out->res_priv;
...
static int
cam_vfe_bus_ver3_handle_bus_irq(uint32_t evt_id,
				struct cam_irq_th_payload *th_payload)
{
	struct cam_vfe_bus_ver3_priv *bus_priv;
	int rc = 0;
	...
	rc = cam_irq_controller_handle_irq(
		evt_id, bus_priv->common_data.bus_irq_controller);
	return (rc == IRQ_HANDLED) ? 0 : -EINVAL;
}

static int
cam_vfe_bus_ver3_handle_rup_irq(uint32_t evt_id,
				struct cam_irq_th_payload *th_payload)
{
	struct cam_vfe_bus_ver3_priv *bus_priv;
	int rc = 0;
	...
	CAM_DBG(CAM_ISP, "Enter");
	rc = cam_irq_controller_handle_irq(
		evt_id, bus_priv->common_data.rup_irq_controller);
	return (rc == IRQ_HANDLED) ? 0 : -EINVAL;
}

static int
cam_vfe_bus_ver3_err_irq_top_half(uint32_t evt_id,
				  struct cam_irq_th_payload *th_payload)
{
	int i = 0, rc = 0;
	struct cam_vfe_bus_ver3_priv *bus_priv = th_payload->handler_priv;
	struct cam_vfe_bus_irq_evt_payload *evt_payload;
...
static int cam_vfe_bus_ver3_err_irq_bottom_half(void *handler_priv,
						void *evt_payload_priv)
{
	struct cam_vfe_bus_irq_evt_payload *evt_payload = evt_payload_priv;
	...
	if (common_data->is_lite) {
		if (image_size_violation) {
			...
			if (val & 0x01) {
				...
				cam_vfe_bus_ver3_print_dimensions(
					CAM_VFE_BUS_VER3_VFE_OUT_RDI0, PLANE_Y,
					bus_priv);
			}
			...
			if (val & 0x02) {
				...
				cam_vfe_bus_ver3_print_dimensions(
					CAM_VFE_BUS_VER3_VFE_OUT_RDI1, PLANE_Y,
					bus_priv);
			}
			...
			if (val & 0x04) {
				...
				cam_vfe_bus_ver3_print_dimensions(
					CAM_VFE_BUS_VER3_VFE_OUT_RDI2, PLANE_Y,
					bus_priv);
			}

			if (val & 0x08) {
				CAM_INFO(CAM_ISP, "RDI 3 image size violation");
				cam_vfe_bus_ver3_print_dimensions(
					CAM_VFE_BUS_VER3_VFE_OUT_RDI3, PLANE_Y,
					bus_priv);
			}
		}
		...
		if (ccif_violation) {
			...
			if (val & 0x08)
				CAM_INFO(CAM_ISP, "RDI 3 ccif violation");
		}
	...
	if (image_size_violation) {
		...
		if (val & 0x01) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_FULL, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x02) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_FULL, PLANE_C,
				bus_priv);
		}
		...
		if (val & 0x04) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_DS4, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x08) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_DS16, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x010) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_FULL_DISP, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x020) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_FULL_DISP, PLANE_C,
				bus_priv);
		}
		...
		if (val & 0x040) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_DS4_DISP, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x080) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_DS16_DISP, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x0100) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_FD, PLANE_Y, bus_priv);
		}

		if (val & 0x0200) {
			CAM_INFO(CAM_ISP, "FD C image size violation");
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_FD, PLANE_C, bus_priv);
		}
		...
		if (val & 0x0400) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_RAW_DUMP, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x01000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_HDR_BE, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x04000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_TL_BG, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x08000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_AWB_BG, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x010000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_BHIST, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x020000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_RS, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x040000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_CS, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x080000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_IHIST, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x0100000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_STATS_BF, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x0200000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_2PD, PLANE_Y,
				bus_priv);
		}
		...
		if (val & 0x0400000) {
			...
			cam_vfe_bus_ver3_print_dimensions(
				CAM_VFE_BUS_VER3_VFE_OUT_LCR, PLANE_Y,
				bus_priv);
		}
...
static void cam_vfe_bus_ver3_update_ubwc_meta_addr(uint32_t *reg_val_pair,
						   uint32_t *j, void *regs,
						   uint64_t image_buf)
{
	struct cam_vfe_bus_ver3_reg_offset_ubwc_client *ubwc_regs;
	...
	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->meta_addr,
				 image_buf);
	...
	uint32_t *reg_val_pair, uint32_t i, uint32_t *j)
{
	...
	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, wm_data->hw_regs->packer_cfg,
				 wm_data->packer_cfg);
	CAM_DBG(CAM_ISP, "WM:%d packer cfg 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->meta_cfg,
				 wm_data->ubwc_meta_cfg);
	CAM_DBG(CAM_ISP, "WM:%d meta stride 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->mode_cfg,
				 wm_data->ubwc_mode_cfg);
	CAM_DBG(CAM_ISP, "WM:%d ubwc_mode_cfg 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->ctrl_2,
				 wm_data->ubwc_ctrl_2);
	CAM_DBG(CAM_ISP, "WM:%d ubwc_ctrl_2 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->lossy_thresh0,
				 wm_data->ubwc_lossy_threshold_0);
	CAM_DBG(CAM_ISP, "WM:%d lossy_thresh0 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->lossy_thresh1,
				 wm_data->ubwc_lossy_threshold_1);
	CAM_DBG(CAM_ISP, "WM:%d lossy_thresh1 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->off_lossy_var,
				 wm_data->ubwc_offset_lossy_variance);
	CAM_DBG(CAM_ISP, "WM:%d off_lossy_var 0x%X", wm_data->index,
		reg_val_pair[*j - 1]);

	if (wm_data->ubwc_bandwidth_limit) {
		CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, *j, ubwc_regs->bw_limit,
					 wm_data->ubwc_bandwidth_limit);
		CAM_DBG(CAM_ISP, "WM:%d ubwc bw limit 0x%X", wm_data->index,
			wm_data->ubwc_bandwidth_limit);
	}
				      ...
				      uint32_t arg_size)
{
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		...
		CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, j, wm_data->hw_regs->cfg,
					 wm_data->en_cfg);
		CAM_DBG(CAM_ISP, "WM:%d en_cfg 0x%X", wm_data->index,
			reg_val_pair[j - 1]);
		...
		if (wm_data->stride != val || !wm_data->init_cfg_done) {
			CAM_VFE_ADD_REG_VAL_PAIR(
				reg_val_pair, j, wm_data->hw_regs->image_cfg_2,
		...
		if (wm_data->en_ubwc) {
			...
			cam_vfe_bus_ver3_update_ubwc_meta_addr(
				reg_val_pair, &j, wm_data->hw_regs->ubwc_regs,
		...
		if (wm_data->en_ubwc) {
			frame_inc =
				ALIGNUP(io_cfg->planes[i].plane_stride *
		...
		if (!(wm_data->en_cfg & (0x3 << 16))) {
			CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
						 ...
						 wm_data->h_init);
			CAM_DBG(CAM_ISP, "WM:%d h_init 0x%X", wm_data->index,
				reg_val_pair[j - 1]);
		}
		...
		CAM_VFE_ADD_REG_VAL_PAIR(reg_val_pair, j,
					 ...
					 frame_inc);
		CAM_DBG(CAM_ISP, "WM:%d frame_inc %d", wm_data->index,
			reg_val_pair[j - 1]);
				       ...
				       uint32_t arg_size)
{
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		...
		if (wm_data->irq_subsample_period !=
			    hfr_cfg->subsample_period ||
		    !wm_data->hfr_cfg_done) {
			CAM_VFE_ADD_REG_VAL_PAIR(
				reg_val_pair, j,
		...
		if (wm_data->irq_subsample_pattern !=
...
static int cam_vfe_bus_ver3_update_ubwc_config_v2(void *cmd_args)
{
	...
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		...
		if (!wm_data->hw_regs->ubwc_regs) {
			CAM_ERR(CAM_ISP, "No UBWC register to configure.");
			rc = -EINVAL;
			goto end;
		}
		...
		if (wm_data->ubwc_ctrl_2 != ubwc_generic_plane_cfg->ctrl_2 ||
		    !wm_data->init_cfg_done) {
			wm_data->ubwc_ctrl_2 = ubwc_generic_plane_cfg->ctrl_2;
			wm_data->ubwc_updated = true;
		}
					      ...
					      uint32_t arg_size)
{
	...
	ports_plane_idx =
		(stripe_args->split_id *
		 (stripe_args->dual_cfg->num_ports * CAM_PACKET_MAX_PLANES)) +
		(outport_id * CAM_PACKET_MAX_PLANES);
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		wm_data = vfe_out_data->wm_res[i]->res_priv;
		stripe_config =
			(struct cam_isp_dual_stripe_config *)&stripe_args
				->dual_cfg->stripes[ports_plane_idx + i];
...
static int cam_vfe_bus_ver3_update_wm_config(void *cmd_args)
{
	int i;
	...
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		...
		CAM_DBG(CAM_ISP, "WM:%d en_cfg:0x%X height:%d width:%d",
...
static int cam_vfe_bus_ver3_start_hw(void *hw_priv, void *start_hw_args,
				     uint32_t arg_size)
{
	return cam_vfe_bus_ver3_start_vfe_out(hw_priv);
}

static int cam_vfe_bus_ver3_stop_hw(void *hw_priv, void *stop_hw_args,
				    uint32_t arg_size)
{
	return cam_vfe_bus_ver3_stop_vfe_out(hw_priv);
}

static int cam_vfe_bus_ver3_init_hw(void *hw_priv, void *init_hw_args,
				    uint32_t arg_size)
{
	...
	bus_priv->bus_irq_handle = cam_irq_controller_subscribe_irq(
		bus_priv->common_data.vfe_irq_controller, CAM_IRQ_PRIORITY_4,
		top_irq_reg_mask, bus_priv, cam_vfe_bus_ver3_handle_bus_irq,
		NULL, NULL, NULL);
	...
	bus_priv->rup_irq_handle = cam_irq_controller_subscribe_irq(
		bus_priv->common_data.vfe_irq_controller, CAM_IRQ_PRIORITY_2,
		top_irq_reg_mask, bus_priv, cam_vfe_bus_ver3_handle_rup_irq,
		NULL, NULL, NULL);
	...
	if (bus_priv->tasklet_info != NULL) {
		bus_priv->error_irq_handle = cam_irq_controller_subscribe_irq(
			...
			CAM_IRQ_PRIORITY_0, bus_error_irq_mask, bus_priv,
			cam_vfe_bus_ver3_err_irq_top_half,
			cam_vfe_bus_ver3_err_irq_bottom_half,
			bus_priv->tasklet_info, &tasklet_bh_api);
...
static int cam_vfe_bus_ver3_deinit_hw(void *hw_priv, void *deinit_hw_args,
				      uint32_t arg_size)
{
	struct cam_vfe_bus_ver3_priv *bus_priv = hw_priv;
	int rc = 0, i;
...
static int __cam_vfe_bus_ver3_process_cmd(void *priv, uint32_t cmd_type,
					...
					uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_STRIPE_UPDATE:
		rc = cam_vfe_bus_ver3_update_stripe_cfg(priv, cmd_args,
							arg_size);
		break;
...
int cam_vfe_bus_ver3_init(struct cam_hw_soc_info *soc_info,
			  ...
			  struct cam_vfe_bus **vfe_bus)
{
	...
	bus_priv = kzalloc(sizeof(struct cam_vfe_bus_ver3_priv), GFP_KERNEL);
	if (!bus_priv) {
	...
	bus_priv->common_data.comp_done_shift = ver3_hw_info->comp_done_shift;
	bus_priv->common_data.hw_init = false;
	...
	for (i = 0; i < bus_priv->num_client; i++) {
		rc = cam_vfe_bus_ver3_init_wm_resource(
			i, bus_priv, bus_hw_info, &bus_priv->bus_client[i]);
		if (rc < 0) {
	...
	for (i = 0; i < CAM_VFE_BUS_VER3_COMP_GRP_MAX; i++) {
		rc = cam_vfe_bus_ver3_init_comp_grp(i, soc_info, bus_priv,
						    bus_hw_info,
						    &bus_priv->comp_grp[i]);
		if (rc < 0) {
...
int cam_vfe_bus_ver3_deinit(struct cam_vfe_bus **vfe_bus)
{
	...
	for (i = 0; i < bus_priv->num_client; i++) {
		...
		if (rc < 0)
			CAM_ERR(CAM_ISP, "VFE:%d deinit WM:%d failed rc:%d",
				bus_priv->common_data.core_index, i, rc);
	}
	...
	rc = cam_irq_controller_deinit(
		&bus_priv->common_data.bus_irq_controller);
	if (rc)
		CAM_ERR(CAM_ISP, "Deinit BUS IRQ Controller failed rc=%d", rc);

	rc = cam_irq_controller_deinit(
		&bus_priv->common_data.rup_irq_controller);
	if (rc)
		CAM_ERR(CAM_ISP, "Deinit RUP IRQ Controller failed rc=%d", rc);
