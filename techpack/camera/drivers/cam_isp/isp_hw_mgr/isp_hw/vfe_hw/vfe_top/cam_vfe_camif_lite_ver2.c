	...
	struct cam_isp_resource_node *camif_lite_res, void *acquire_param)
{
	...
	CAM_DBG(CAM_ISP, "hw id:%d sync_mode=%d",
		camif_lite_res->hw_intf->hw_idx, camif_lite_data->sync_mode);
	return rc;
}
...
cam_vfe_camif_lite_resource_start(struct cam_isp_resource_node *camif_lite_res)
{
	...
	CAM_DBG(CAM_ISP, "hw id:%d RUP val:%d", camif_lite_res->hw_intf->hw_idx,
...
int cam_vfe_camif_lite_ver2_init(struct cam_hw_intf *hw_intf,
				 struct cam_hw_soc_info *soc_info,
...
int cam_vfe_camif_lite_ver2_deinit(struct cam_isp_resource_node *camif_lite_node)
{
	struct cam_vfe_mux_camif_lite_data *camif_lite_priv =
