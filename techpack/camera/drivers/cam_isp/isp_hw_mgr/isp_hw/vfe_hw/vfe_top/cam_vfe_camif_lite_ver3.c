	...
	struct cam_vfe_top_irq_evt_payload **evt_payload)
{
	...
	*evt_payload =
		list_first_entry(&camif_lite_priv->free_payload_list,
...
static int
cam_vfe_camif_lite_err_irq_top_half(uint32_t evt_id,
				    struct cam_irq_th_payload *th_payload)
{
	...
	if (th_payload->evt_status_arr[2] ||
		...
		CAM_ERR(CAM_ISP,
			...
			camif_lite_node->res_id, th_payload->evt_status_arr[0],
		...
		cam_irq_controller_clear_and_mask(
			evt_id, camif_lite_priv->vfe_irq_controller);
		error_flag = true;
	}
...
static int
cam_vfe_camif_lite_get_reg_update(struct cam_isp_resource_node *camif_lite_res,
				  void *cmd_args, uint32_t arg_size)
{
	...
	if (!cdm_args || !cdm_args->res) {
		CAM_ERR(CAM_ISP, "Invalid args: cdm args %pK", cdm_args);
		return -EINVAL;
	}
...
int cam_vfe_camif_lite_ver3_acquire_resource(
	struct cam_isp_resource_node *camif_lite_res, void *acquire_param)
{
	...
	CAM_DBG(CAM_ISP, "Acquired VFE:%d CAMIF LITE:%d sync_mode=%d",
		camif_lite_res->hw_intf->hw_idx, camif_lite_res->res_id,
		camif_lite_data->sync_mode);
	return 0;
}

static int
cam_vfe_camif_lite_resource_start(struct cam_isp_resource_node *camif_lite_res)
{
...
skip_core_cfg:
	...
	if (!rsrc_data->irq_handle) {
		rsrc_data->irq_handle = cam_irq_controller_subscribe_irq(
			...
			camif_lite_res->tasklet_info, &tasklet_bh_api);
...
static int
cam_vfe_camif_lite_reg_dump(struct cam_isp_resource_node *camif_lite_res)
{
	struct cam_vfe_mux_camif_lite_data *camif_lite_priv;
	struct cam_vfe_soc_private *soc_private = NULL;
	...
	if (!soc_private->is_ife_lite) {
		for (offset = 0x0; offset <= 0x1FC; offset += 0x4) {
			if (offset == 0x1C || offset == 0x34 ||
				...
				continue;
			val = cam_soc_util_r(camif_lite_priv->soc_info, 0,
					     offset);
...
dump_pdlib:
	...
	CAM_INFO(CAM_ISP, "IFE:%d CLC PDLIB", camif_lite_priv->hw_intf->hw_idx);
...
wr_dump:
	...
	for (wm_idx = 0; wm_idx <= 3; wm_idx++) {
		for (offset = 0x1C00 + 0x100 * wm_idx;
		     offset < (0x1C00 + 0x100 * wm_idx + 0x84); offset += 0x4) {
			val = cam_soc_util_r(camif_lite_priv->soc_info, 0,
					     offset);
			CAM_INFO(CAM_ISP, "offset 0x%X value 0x%X", offset,
				 val);
...
static int
cam_vfe_camif_lite_resource_stop(struct cam_isp_resource_node *camif_lite_res)
{
	struct cam_vfe_mux_camif_lite_data *rsrc_data;
	int rc = 0;
	...
	if (rsrc_data->irq_handle > 0) {
		cam_irq_controller_unsubscribe_irq(
			rsrc_data->vfe_irq_controller, rsrc_data->irq_handle);
		rsrc_data->irq_handle = 0;
	}
...
static int
cam_vfe_camif_lite_ver3_core_config(struct cam_isp_resource_node *rsrc_node,
				    void *cmd_args)
{
	struct cam_vfe_mux_camif_lite_data *camif_lite_priv;
...
static int
cam_vfe_camif_lite_process_cmd(struct cam_isp_resource_node *rsrc_node,
...
static void cam_vfe_camif_lite_print_status(
	uint32_t *status, int err_type,
	struct cam_vfe_mux_camif_lite_data *camif_lite_priv)
{
...
static int
cam_vfe_camif_lite_handle_irq_top_half(uint32_t evt_id,
				       struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	int i;
...
static int cam_vfe_camif_lite_handle_irq_bottom_half(void *handler_priv,
...
int cam_vfe_camif_lite_ver3_init(struct cam_hw_intf *hw_intf,
				 struct cam_hw_soc_info *soc_info,
				 ...
				 void *vfe_irq_controller)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%d CAMIF LITE:%d Init", camif_lite_node->res_id,
		camif_lite_node->res_id);

	camif_lite_priv = kzalloc(sizeof(*camif_lite_priv), GFP_KERNEL);
	if (!camif_lite_priv)
		return -ENOMEM;
...
int cam_vfe_camif_lite_ver3_deinit(struct cam_isp_resource_node *camif_lite_node)
{
	struct cam_vfe_mux_camif_lite_data *camif_lite_priv =
