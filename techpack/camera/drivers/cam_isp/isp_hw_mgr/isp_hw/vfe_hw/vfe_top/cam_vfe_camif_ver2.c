...
static int
cam_vfe_camif_get_evt_payload(struct cam_vfe_mux_camif_data *camif_priv,
...
static int
cam_vfe_camif_put_evt_payload(struct cam_vfe_mux_camif_data *camif_priv,
			      struct cam_vfe_top_irq_evt_payload **evt_payload)
{
	unsigned long flags;
...
static int cam_vfe_camif_err_irq_top_half(uint32_t evt_id,
...
static int cam_vfe_camif_get_reg_update(struct cam_isp_resource_node *camif_res,
					void *cmd_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "CAMIF reg_update_cmd %x offset %x", reg_val_pair[1],
		reg_val_pair[0]);
...
int cam_vfe_camif_ver2_acquire_resource(struct cam_isp_resource_node *camif_res,
					void *acquire_param)
{
...
static int cam_vfe_camif_resource_init(struct cam_isp_resource_node *camif_res,
...
static int
cam_vfe_camif_resource_deinit(struct cam_isp_resource_node *camif_res,
			      void *init_args, uint32_t arg_size)
{
	struct cam_vfe_mux_camif_data *camif_data;
	struct cam_hw_soc_info *soc_info;
...
static int cam_vfe_camif_resource_start(struct cam_isp_resource_node *camif_res)
{
	...
	if (!rsrc_data->irq_err_handle) {
		rsrc_data->irq_err_handle = cam_irq_controller_subscribe_irq(
			...
			err_irq_mask, camif_res, cam_vfe_camif_err_irq_top_half,
			camif_res->bottom_half_handler, camif_res->tasklet_info,
			&tasklet_bh_api);
...
static int cam_vfe_camif_reg_dump(struct cam_isp_resource_node *camif_res)
{
	struct cam_vfe_mux_camif_data *camif_priv;
	struct cam_vfe_soc_private *soc_private;
	...
	if (soc_private->cpas_version == CAM_CPAS_TITAN_175_V120 ||
		...
		cam_cpas_reg_read(soc_private->cpas_handle, CAM_CPAS_REG_CAMNOC,
				  0x3620, true, &val);
		CAM_INFO(CAM_ISP,
			 "IFE0123_RDI_WR_MAXWR_LOW offset 0x3620 val 0x%x",
			 val);
...
static int cam_vfe_camif_resource_stop(struct cam_isp_resource_node *camif_res)
{
	struct cam_vfe_mux_camif_data *camif_priv;
	struct cam_vfe_camif_ver2_reg *camif_reg;
	int rc = 0;
...
static int cam_vfe_camif_sof_irq_debug(struct cam_isp_resource_node *rsrc_node,
				       void *cmd_args)
{
	struct cam_vfe_mux_camif_data *camif_priv;
...
static int
cam_vfe_camif_handle_irq_top_half(uint32_t evt_id,
				  struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	int i;
						...
						void *evt_payload_priv)
{
	...
	CAM_DBG(CAM_ISP, "irq_status_0 = 0x%x irq_status_1 = 0x%x", irq_status0,
		irq_status1);
...
int cam_vfe_camif_ver2_init(struct cam_hw_intf *hw_intf,
			    struct cam_hw_soc_info *soc_info,
			    void *camif_hw_info,
			    struct cam_isp_resource_node *camif_node,
			    void *vfe_irq_controller)
{
	...
	camif_priv = kzalloc(sizeof(struct cam_vfe_mux_camif_data), GFP_KERNEL);
	if (!camif_priv) {
...
int cam_vfe_camif_ver2_deinit(struct cam_isp_resource_node *camif_node)
{
