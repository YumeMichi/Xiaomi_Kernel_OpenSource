...
static int
cam_vfe_rdi_get_evt_payload(struct cam_vfe_mux_rdi_data *rdi_priv,
			    struct cam_vfe_top_irq_evt_payload **evt_payload)
{
	...
	*evt_payload =
		list_first_entry(&rdi_priv->free_payload_list,
...
static int
cam_vfe_rdi_put_evt_payload(struct cam_vfe_mux_rdi_data *rdi_priv,
			    struct cam_vfe_top_irq_evt_payload **evt_payload)
{
	unsigned long flags;
...
static int cam_vfe_rdi_err_irq_top_half(uint32_t evt_id,
					struct cam_irq_th_payload *th_payload)
{
	...
	if (th_payload->evt_status_arr[1]) {
		CAM_ERR(CAM_ISP, "RDI Error: vfe:%d: STATUS_1=0x%x",
	...
	evt_payload->irq_reg_val[i] = cam_io_r(
		rdi_priv->mem_base + rdi_priv->common_reg->violation_status);
...
static int cam_vfe_rdi_get_reg_update(struct cam_isp_resource_node *rdi_res,
...
int cam_vfe_rdi_ver2_acquire_resource(struct cam_isp_resource_node *rdi_res,
				      void *acquire_param)
{
...
static int cam_vfe_rdi_resource_start(struct cam_isp_resource_node *rdi_res)
{
	...
	if (!rsrc_data->irq_err_handle) {
		rsrc_data->irq_err_handle = cam_irq_controller_subscribe_irq(
			...
			err_irq_mask, rdi_res, cam_vfe_rdi_err_irq_top_half,
			rdi_res->bottom_half_handler, rdi_res->tasklet_info,
			&tasklet_bh_api);
	...
	if (!rsrc_data->irq_handle) {
		rsrc_data->irq_handle = cam_irq_controller_subscribe_irq(
			...
			irq_mask, rdi_res, rdi_res->top_half_handler,
			rdi_res->bottom_half_handler, rdi_res->tasklet_info,
			&tasklet_bh_api);
...
static int cam_vfe_rdi_resource_stop(struct cam_isp_resource_node *rdi_res)
{
	struct cam_vfe_mux_rdi_data *rdi_priv;
	int rc = 0;
	...
	if (rdi_priv->irq_handle) {
		cam_irq_controller_unsubscribe_irq(rdi_priv->vfe_irq_controller,
						   rdi_priv->irq_handle);
				   ...
				   uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_GET_REG_UPDATE:
		rc = cam_vfe_rdi_get_reg_update(rsrc_node, cmd_args, arg_size);
		break;
	default:
		CAM_ERR(CAM_ISP, "unsupported RDI process command:%d",
			cmd_type);
		break;
	}
...
static int
cam_vfe_rdi_handle_irq_top_half(uint32_t evt_id,
				struct cam_irq_th_payload *th_payload)
{
	int32_t rc;
	int i;
					      ...
					      void *evt_payload_priv)
{
	...
	if (irq_status0 & rdi_priv->reg_data->sof_irq_mask) {
		...
		if (rdi_priv->event_cb)
			rdi_priv->event_cb(rdi_priv->priv, CAM_ISP_HW_EVENT_SOF,
					   (void *)&evt_info);
...
int cam_vfe_rdi_ver2_init(struct cam_hw_intf *hw_intf,
			  struct cam_hw_soc_info *soc_info, void *rdi_hw_info,
			  struct cam_isp_resource_node *rdi_node,
			  void *vfe_irq_controller)
{
	...
	rdi_priv = kzalloc(sizeof(struct cam_vfe_mux_rdi_data), GFP_KERNEL);
	if (!rdi_priv) {
...
int cam_vfe_rdi_ver2_deinit(struct cam_isp_resource_node *rdi_node)
{
