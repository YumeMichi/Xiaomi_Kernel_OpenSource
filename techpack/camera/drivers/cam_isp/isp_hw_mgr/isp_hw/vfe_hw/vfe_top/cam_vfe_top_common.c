...
static struct cam_axi_vote *
cam_vfe_top_delay_bw_reduction(struct cam_vfe_top_priv_common *top_common,
			       uint64_t *to_be_applied_bw)
{
	...
	for (i = 0; i < CAM_VFE_DELAY_BW_REDUCTION_NUM_FRAMES; i++) {
		...
		for (j = 0; j < curr_l_vote->num_paths; j++) {
			if (total_bw >
			    (U64_MAX - curr_l_vote->axi_path[j].camnoc_bw)) {
				CAM_ERR(CAM_PERF,
				...
				bool start_stop)
{
	...
	for (i = 0; i < top_common->num_mux; i++) {
		if (top_common->axi_vote_control[i] ==
			...
			if (num_paths + top_common->req_axi_vote[i].num_paths >
			    CAM_CPAS_MAX_PATHS_PER_CLIENT) {
				CAM_ERR(CAM_PERF,
					"Required paths(%d) more than max(%d)",
					num_paths + top_common->req_axi_vote[i]
							    .num_paths,
					CAM_CPAS_MAX_PATHS_PER_CLIENT);
				return -EINVAL;
			}
	...
	for (i = 0; i < agg_vote.num_paths; i++) {
		CAM_DBG(CAM_PERF,
			...
			top_common->hw_idx, top_common->last_counter,
	...
	if (start_stop) {
		 ...
		 */
		to_be_applied_axi_vote = cam_vfe_top_delay_bw_reduction(
			top_common, &total_bw_new_vote);
	...
	for (i = 0; i < to_be_applied_axi_vote->num_paths; i++) {
		CAM_DBG(CAM_PERF,
			...
			top_common->hw_idx,
			cam_cpas_axi_util_path_type_to_string(
				to_be_applied_axi_vote->axi_path[i]
					.path_data_type),
			cam_cpas_axi_util_trans_type_to_string(
				to_be_applied_axi_vote->axi_path[i]
					.transac_type),
			  ...
			  void *cmd_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_PERF, "res_id=%d, BW=[%lld %lld]", res->res_id,
		bw_update->camnoc_bw_bytes, bw_update->external_bw_bytes);
