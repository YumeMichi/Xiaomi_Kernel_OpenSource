...
static int cam_vfe_top_set_hw_clk_rate(struct cam_vfe_top_ver2_priv *top_priv)
{
...
static int cam_vfe_top_fs_update(struct cam_vfe_top_ver2_priv *top_priv,
				 void *cmd_args, uint32_t arg_size)
{
...
static int cam_vfe_top_clock_update(struct cam_vfe_top_ver2_priv *top_priv,
				    void *cmd_args, uint32_t arg_size)
{
...
static int
cam_vfe_top_mux_get_reg_update(struct cam_vfe_top_ver2_priv *top_priv,
			       void *cmd_args, uint32_t arg_size)
{
...
int cam_vfe_top_get_hw_caps(void *device_priv, void *get_hw_cap_args,
			    uint32_t arg_size)
{
	return -EPERM;
}

int cam_vfe_top_init_hw(void *device_priv, void *init_hw_args,
			uint32_t arg_size)
{
...
int cam_vfe_top_reset(void *device_priv, void *reset_core_args,
		      uint32_t arg_size)
{
	struct cam_vfe_top_ver2_priv *top_priv = device_priv;
	struct cam_hw_soc_info *soc_info = NULL;
...
int cam_vfe_top_reserve(void *device_priv, void *reserve_args,
			uint32_t arg_size)
{
	...
	for (i = 0; i < top_priv->top_common.num_mux; i++) {
		if (top_priv->top_common.mux_rsrc[i].res_id ==
			...
			if (acquire_args->res_id == CAM_ISP_HW_VFE_IN_PDLIB) {
			...
			if ((acquire_args->res_id >= CAM_ISP_HW_VFE_IN_RDI0) &&
			    (acquire_args->res_id <= CAM_ISP_HW_VFE_IN_RDI3)) {
...
int cam_vfe_top_release(void *device_priv, void *release_args,
			uint32_t arg_size)
{
	struct cam_vfe_top_ver2_priv *top_priv;
...
int cam_vfe_top_start(void *device_priv, void *start_args, uint32_t arg_size)
{
	struct cam_vfe_top_ver2_priv *top_priv;
	struct cam_isp_resource_node *mux_res;
	struct cam_hw_info *hw_info = NULL;
...
int cam_vfe_top_stop(void *device_priv, void *stop_args, uint32_t arg_size)
{
	struct cam_vfe_top_ver2_priv *top_priv;
	struct cam_isp_resource_node *mux_res;
	struct cam_hw_info *hw_info = NULL;
...
int cam_vfe_top_read(void *device_priv, void *read_args, uint32_t arg_size)
{
	return -EPERM;
}

int cam_vfe_top_write(void *device_priv, void *write_args, uint32_t arg_size)
{
	return -EPERM;
}
			    ...
			    void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_CLOCK_UPDATE:
		rc = cam_vfe_top_clock_update(top_priv, cmd_args, arg_size);
		break;
			  ...
			  struct cam_vfe_top **vfe_top_ptr)
{
	...
	top_priv = kzalloc(sizeof(struct cam_vfe_top_ver2_priv), GFP_KERNEL);
	if (!top_priv) {
	...
	for (i = 0, j = 0; i < top_priv->top_common.num_mux; i++) {
		...
		if (ver2_hw_info->mux_type[i] == CAM_VFE_CAMIF_VER_2_0) {
			...
			rc = cam_vfe_camif_ver2_init(
				hw_intf, soc_info, &ver2_hw_info->camif_hw_info,
				&top_priv->top_common.mux_rsrc[i],
				vfe_irq_controller);
			if (rc)
			...
			rc = cam_vfe_camif_lite_ver2_init(
				hw_intf, soc_info,
			...
			if (rc)
				goto deinit_resources;
		} else if (ver2_hw_info->mux_type[i] == CAM_VFE_RDI_VER_1_0) {
			/* set the RDI resource id */
			top_priv->top_common.mux_rsrc[i].res_id =
				CAM_ISP_HW_VFE_IN_RDI0 + j++;

			rc = cam_vfe_rdi_ver2_init(
				hw_intf, soc_info, &ver2_hw_info->rdi_hw_info,
				&top_priv->top_common.mux_rsrc[i],
				vfe_irq_controller);
			if (rc)
				goto deinit_resources;
		} else if (ver2_hw_info->mux_type[i] == CAM_VFE_IN_RD_VER_1_0) {
			/* set the RD resource id */
			top_priv->top_common.mux_rsrc[i].res_id =
				CAM_ISP_HW_VFE_IN_RD;

			rc = cam_vfe_fe_ver1_init(
				hw_intf, soc_info, &ver2_hw_info->fe_hw_info,
				&top_priv->top_common.mux_rsrc[i]);
			if (rc)
				goto deinit_resources;
		} else {
