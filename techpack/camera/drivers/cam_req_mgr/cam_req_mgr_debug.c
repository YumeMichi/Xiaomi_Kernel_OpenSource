...
static int cam_req_mgr_debug_set_bubble_recovery(void *data, u64 val)
{
	...
	if (!list_empty(&core_dev->session_head)) {
		list_for_each_entry (session, &core_dev->session_head, entry) {
...
static ssize_t session_info_write(struct file *t_file, const char *t_char,
				  size_t t_size_t, loff_t *t_loff_t)
{
...
int cam_req_mgr_debug_register(struct cam_req_mgr_core_device *core_dev)
{
	...
	if (!debugfs_create_file("sessions_info", 0644, debugfs_root, core_dev,
				 &session_info))
		return -ENOMEM;

	if (!debugfs_create_file("bubble_recovery", 0644, debugfs_root,
				 core_dev, &bubble_recovery))
		return -ENOMEM;
