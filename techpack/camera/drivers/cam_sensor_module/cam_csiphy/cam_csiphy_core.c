...
int32_t cam_csiphy_get_instance_offset(struct csiphy_device *csiphy_dev,
				       int32_t dev_handle)
{
	int32_t i;
	...
	for (i = 0; i < csiphy_dev->acquire_count; i++) {
		if (dev_handle == csiphy_dev->bridge_intf.device_hdl[i])
			break;
	}
...
void cam_csiphy_reset(struct csiphy_device *csiphy_dev)
{
	...
	for (i = 0; i < size; i++) {
		cam_io_w_mb(csiphy_dev->ctrl_reg->csiphy_reset_reg[i].reg_data,
			    base + csiphy_dev->ctrl_reg->csiphy_reset_reg[i]
					    .reg_addr);
			      ...
			      struct cam_config_dev_cmd *cfg_dev)
{
	...
	offset =
		cam_csiphy_get_instance_offset(csiphy_dev, cfg_dev->dev_handle);
	...
	adj_lane_mask =
		cam_cmd_csiphy_info->lane_mask & LANE_MASK_2PH & ~clock_lane;
	temp = adj_lane_mask & (clock_lane - 1);
	adj_lane_mask = ((adj_lane_mask & (~(clock_lane - 1))) >> 1) | temp;
	...
	csiphy_dev->csiphy_cpas_cp_reg_mask[offset] =
		adj_lane_mask
		<< (csiphy_dev->soc_info.index * (CAM_CSIPHY_MAX_DPHY_LANES +
			   ...
			   struct cam_config_dev_cmd *cfg_dev)
{
	...
	csl_packet = (struct cam_packet *)(generic_pkt_ptr +
					   (uint32_t)cfg_dev->offset);
	...
	cmd_desc =
		(struct cam_cmd_buf_desc *)((uint32_t *)&csl_packet->payload +
					    csl_packet->cmd_buf_offset / 4);

	rc = cam_mem_get_cpu_buf(cmd_desc->mem_handle, &generic_ptr, &len);
	if (rc < 0) {
		CAM_ERR(CAM_CSIPHY, "Failed to get cmd buf Mem address : %d",
			rc);
		return rc;
	}
	...
	if (cam_cmd_csiphy_info->secure_mode == 1)
		cam_csiphy_update_secure_info(csiphy_dev, cam_cmd_csiphy_info,
					      cfg_dev);
...
{
	int32_t i;
	void __iomem *csiphybase = csiphy_dev->soc_info.reg_map[0].mem_base;

	for (i = 0; i < csiphy_dev->num_irq_registers; i++)
		cam_io_w_mb(csiphy_dev->ctrl_reg->csiphy_irq_reg[i].reg_data,
			    csiphybase + csiphy_dev->ctrl_reg->csiphy_irq_reg[i]
						 .reg_addr);
}
...
void cam_csiphy_cphy_data_rate_config(struct csiphy_device *csiphy_device)
{
	...
	num_table_entries = settings_table->num_data_rate_settings;
	...
	for (i = 0; i < num_table_entries; i++) {
		...
		uint64_t bandwidth = drate_settings[i].bandwidth;
		ssize_t num_reg_entries =
			drate_settings[i].data_rate_reg_array_size;

		if (phy_data_rate > bandwidth) {
			CAM_DBG(CAM_CSIPHY,
				"Skipping table [%d] %llu required: %llu", i,
				bandwidth, phy_data_rate);
			continue;
		}

		CAM_DBG(CAM_CSIPHY, "table[%d] BW : %llu Selected", i,
			bandwidth);
		for (j = 0; j < num_reg_entries; j++) {
			uint32_t reg_addr = drate_settings[i]
						    .csiphy_data_rate_regs[j]
						    .reg_addr;

			uint32_t reg_data = drate_settings[i]
						    .csiphy_data_rate_regs[j]
						    .reg_data;
...
irqreturn_t cam_csiphy_irq(int irq_num, void *data)
{
	uint32_t irq;
	uint8_t i;
	struct csiphy_device *csiphy_dev = (struct csiphy_device *)data;
	...
	for (i = 0; i < csiphy_dev->num_irq_registers; i++) {
		...
		cam_io_w_mb(
			irq,
			base + csiphy_reg->mipi_csiphy_interrupt_clear0_addr +
				(0x4 * i));
...
int32_t cam_csiphy_config_dev(struct csiphy_device *csiphy_dev)
{
	...
	while (lane_mask) {
		...
		for (i = 0; i < cfg_size; i++) {
			...
			case CSIPHY_LANE_ENABLE:
				cam_io_w_mb(lane_enable,
					    csiphybase + reg_array[lane_pos][i]
								 .reg_addr);
				break;
			case CSIPHY_DEFAULT_PARAMS:
			...
			if (reg_array[lane_pos][i].delay > 0) {
				usleep_range(
					reg_array[lane_pos][i].delay * 1000,
					reg_array[lane_pos][i].delay * 1000 +
						10);
			}
		}
				       ...
				       struct cam_config_dev_cmd *p_submit_cmd)
{
	...
	if (copy_from_user(&cam_cmd_csiphy_info,
		...
		CAM_DBG(CAM_CSIPHY,
			...
			__func__, csiphy_dev->csiphy_info.settle_time,
...
int32_t cam_csiphy_core_cfg(void *phy_dev, void *arg)
{
	...
	case CAM_ACQUIRE_DEV: {
		...
		if (copy_to_user(u64_to_user_ptr(cmd->handle), &csiphy_acq_dev,
	...
	case CAM_QUERY_CAP: {
		...
		if (copy_to_user(u64_to_user_ptr(cmd->handle), &csiphy_cap,
	...
	case CAM_STOP_DEV: {
		...
		if (csiphy_dev->csiphy_info.secure_mode[offset])
			cam_csiphy_notify_secure_mode(
				csiphy_dev, CAM_SECURE_MODE_NON_SECURE, offset);
	...
	case CAM_START_DEV: {
		...
		rc = cam_cpas_start(csiphy_dev->cpas_handle, &ahb_vote,
				    &axi_vote);
		if (rc < 0) {
		...
		if (csiphy_dev->csiphy_info.secure_mode[offset] == 1) {
			...
			rc = cam_csiphy_notify_secure_mode(
				csiphy_dev, CAM_SECURE_MODE_SECURE, offset);
			if (rc < 0) {
				csiphy_dev->csiphy_info.secure_mode[offset] =
					CAM_SECURE_MODE_NON_SECURE;
				cam_cpas_stop(csiphy_dev->cpas_handle);
	...
	case CAM_CONFIG_DEV_EXTERNAL: {
		...
		if (copy_from_user(&submit_cmd, u64_to_user_ptr(cmd->handle),
