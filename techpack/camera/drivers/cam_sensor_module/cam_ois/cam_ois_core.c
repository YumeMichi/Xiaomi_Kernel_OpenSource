 ...
 */
static int cam_ois_get_dev_handle(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	ois_acq_dev.device_handle = cam_create_device_hdl(&bridge_params);
...
static int cam_ois_power_up(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if ((power_info->power_setting == NULL) &&
		...
		CAM_INFO(CAM_OIS, "Using default power settings");
					...
					uint32_t *cmd_buf, size_t len)
{
	...
	if (o_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		o_ctrl->is_ois_pre_init =
			ois_info->is_ois_pre_init; //xiaomi add
		...
		CAM_DBG(CAM_OIS,
			"Slave addr: 0x%x Freq Mode: %d, disable optmz %d",
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	rc = cam_mem_get_cpu_buf(dev_config.packet_handle, &generic_pkt_addr,
				 &pkt_len);
	if (rc) {
		CAM_ERR(CAM_OIS, "error in converting command Handle Error: %d",
			rc);
		return rc;
	}
	...
	case CAM_OIS_PACKET_OPCODE_INIT:
			...
			default:
				if (o_ctrl->i2c_init_data.is_settings_valid ==
					...
					rc = cam_sensor_i2c_command_parser(
						&o_ctrl->io_master_info,
						i2c_reg_settings, &cmd_desc[i],
						1);
					if (rc < 0) {
						CAM_ERR(CAM_OIS,
							"init parsing failed: %d",
							rc);
						return rc;
					}
					...
					if (rc < 0) {
						CAM_ERR(CAM_OIS,
							"Calib parsing failed: %d",
							rc);
						return rc;
					}
				} else if ((o_ctrl->is_ois_pre_init !=
					    0) && //xiaomi add begin
					...
					if (rc < 0) {
						CAM_ERR(CAM_OIS,
							"pre init settings parsing failed: %d",
							rc);
						return rc;
					}
				} //xiaomi add end
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "Cannot apply Init settings: rc = %d",
				rc);
			goto pwr_dwn;
		}
		...
		if (rc < 0) {
			CAM_WARN(CAM_OIS, "Fail deleting Pre Init data: rc: %d",
				 rc);
			rc = 0;
		} //xiaomi add end
		...
		if (rc < 0) {
			CAM_WARN(CAM_OIS, "Fail deleting Init data: rc: %d",
				 rc);
			rc = 0;
		}
	...
	case CAM_OIS_PACKET_OPCODE_OIS_CONTROL:
		...
		i2c_reg_settings->request_id = 0;
		rc = cam_sensor_i2c_command_parser(
			&o_ctrl->io_master_info, i2c_reg_settings, cmd_desc, 1);
		if (rc < 0) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "Fail deleting Mode data: rc: %d", rc);
			return rc;
		}
		break;
#ifdef ENABLE_OIS_EIS
	case CAM_OIS_PACKET_OPCODE_OIS_GETDATA:
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "Fail ois_get_data: rc: %d", rc);
			return rc;
		}
		break;
#endif
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	if (cmd->handle_type != CAM_HANDLE_USER_POINTER) {
		CAM_ERR(CAM_OIS, "Invalid handle type: %d", cmd->handle_type);
		return -EINVAL;
	}
	...
	case CAM_QUERY_CAP:
		...
		if (copy_to_user(u64_to_user_ptr(cmd->handle), &ois_cap,
	...
	case CAM_RELEASE_DEV:
		if (o_ctrl->cam_ois_state == CAM_OIS_START) {
			rc = -EINVAL;
			CAM_WARN(CAM_OIS, "Cant release ois: in start state");
			goto release_mutex;
		}
	...
	case CAM_STOP_DEV:
		if (o_ctrl->cam_ois_state != CAM_OIS_START) {
			rc = -EINVAL;
			CAM_WARN(CAM_OIS, "Not in right state for stop : %d",
				 o_ctrl->cam_ois_state);
		}
		o_ctrl->cam_ois_state = CAM_OIS_CONFIG;
		break;
