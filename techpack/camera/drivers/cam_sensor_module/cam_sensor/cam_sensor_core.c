...
static void
cam_sensor_release_per_frame_resource(struct cam_sensor_ctrl_t *s_ctrl)
{
	struct i2c_settings_array *i2c_set = NULL;
	int i, rc;
				     ...
				     struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->pipeline_delay = probe_info->reserved;
					 ...
					 size_t remain_len)
{
	...
	case 0: {
		...
		if (remain_len < (sizeof(struct cam_cmd_i2c_info) +
	...
	case 1: {
		rc = cam_sensor_update_power_settings(
			...
			&s_ctrl->sensordata->power_info, remain_len);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
...
int32_t cam_handle_mem_ptr(uint64_t handle, struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	for (i = 0; i < pkt->num_cmd_buf; i++) {
		if (!(cmd_desc[i].length))
			continue;
		rc = cam_mem_get_cpu_buf(cmd_desc[i].mem_handle, &cmd_buf1,
					 &len);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
		...
		rc = cam_handle_cmd_buffers_for_probe(
			ptr, s_ctrl, i, cmd_desc[i].length, remain_len);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"Failed to parse the command Buffer Header");
			goto end;
...
void cam_sensor_shutdown(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->bridge_intf.device_hdl != -1) {
		...
		if (rc < 0)
			CAM_ERR(CAM_SENSOR, "dhdl already destroyed: rc = %d",
				rc);
	}
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
		CAM_WARN(CAM_SENSOR, "read id: 0x%x expected id 0x%x:", chipid,
			 slave_info->sensor_id);
		return -ENODEV;
	}
	return rc;
...
int32_t cam_sensor_driver_cmd(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
	...
	case CAM_ACQUIRE_DEV: {
		...
		if ((s_ctrl->is_probe_succeed == 0) ||
		    (s_ctrl->sensor_state != CAM_SENSOR_INIT)) {
			CAM_WARN(CAM_SENSOR, "Not in right state to aquire %d",
				 s_ctrl->sensor_state);
			rc = -EINVAL;
			goto release_mutex;
		}
		...
		CAM_DBG(CAM_SENSOR, "Device Handle: %d",
			sensor_acq_dev.device_handle);
		if (copy_to_user(u64_to_user_ptr(cmd->handle), &sensor_acq_dev,
	...
	case CAM_RELEASE_DEV: {
		...
		if (s_ctrl->sensor_state == CAM_SENSOR_START) {
			CAM_WARN(CAM_SENSOR,
				 ...
				 s_ctrl->sensor_state);
			if (s_ctrl->i2c_data.streamoff_settings
				    .is_settings_valid &&
	...
	case CAM_CONFIG_DEV: {
		...
		if (s_ctrl->i2c_data.init_settings.is_settings_valid &&
			...
			pkt_opcode =
				CAM_SENSOR_PACKET_OPCODE_SENSOR_INITIAL_CONFIG;
			rc = cam_sensor_apply_settings(s_ctrl, 0, pkt_opcode);
	...
	case CAM_READ_REG: {
		...
		if (copy_to_user((void __user *)(cmd->handle),
				 &user_reg_setting, sizeof(user_reg_setting)) ||
		    ret != 0) {
			CAM_ERR(CAM_SENSOR, "Copy data to user space failed");
			rc = -EFAULT;
		}
...
int cam_sensor_publish_dev_info(struct cam_req_mgr_device_info *info)
{
	...
	s_ctrl = (struct cam_sensor_ctrl_t *)cam_get_device_priv(info->dev_hdl);
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->bob_pwm_switch) {
		rc = cam_sensor_bob_pwm_mode_switch(
			soc_info, s_ctrl->bob_reg_index, true);
		if (rc) {
			CAM_WARN(CAM_SENSOR, "BoB PWM setup failed rc: %d", rc);
			rc = 0;
		}
	}
...
int cam_sensor_power_down(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->bob_pwm_switch) {
		rc = cam_sensor_bob_pwm_mode_switch(
			soc_info, s_ctrl->bob_reg_index, false);
		if (rc) {
			CAM_WARN(CAM_SENSOR, "BoB PWM setup failed rc: %d", rc);
			rc = 0;
		}
