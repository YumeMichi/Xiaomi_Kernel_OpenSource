...
static int32_t cam_sensor_driver_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	rc = cam_sensor_util_init_gpio_pin_tbl(
		soc_info, &sensordata->power_info.gpio_num_info);
	if (rc < 0) {
	...
	for (i = 0; i < soc_info->num_rgltr; i++) {
		if (!strcmp(soc_info->rgltr_name[i], "cam_bob")) {
			CAM_DBG(CAM_SENSOR, "i: %d cam_bob", i);
...
int32_t msm_sensor_init_default_params(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_DBG(CAM_SENSOR, "master_type: %d",
		s_ctrl->io_master_info.master_type);
	/* Initialize cci_client */
	if (s_ctrl->io_master_info.master_type == CCI_MASTER) {
		s_ctrl->io_master_info.cci_client = kzalloc(
			sizeof(struct cam_sensor_cci_client), GFP_KERNEL);
		if (!(s_ctrl->io_master_info.cci_client))
			return -ENOMEM;

		s_ctrl->io_master_info.cci_client->cci_device = s_ctrl->cci_num;
