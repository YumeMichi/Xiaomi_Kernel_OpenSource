...
static int32_t
cam_cci_i2c_write_table_cmd(struct camera_io_master *client,
			    ...
			    enum cam_cci_cmd_type cmd)
{
	...
	cci_ctrl.cfg.cci_i2c_write_cfg.reg_setting = write_setting->reg_setting;
...
int32_t
cam_cci_i2c_write_table(struct camera_io_master *client,
			struct cam_sensor_i2c_reg_setting *write_setting)
{
				   ...
				   enum camera_sensor_i2c_type addr_type)
{
	...
	rc = cam_cci_i2c_read(client, addr, &reg_data, addr_type, data_type);
	if (rc < 0)
		return rc;
	//add by baichunyu@xiaomi.com
	CAM_DBG(CAM_SENSOR, "addr %04x, %04x,compare data = %d", addr, reg_data,
		(int16_t)reg_data);
	//add by baichunyu@xiaomi.com
...
int32_t cam_cci_i2c_poll(struct cam_sensor_cci_client *client, uint32_t addr,
			 ...
			 uint32_t delay_ms)
{
	...
	CAM_DBG(CAM_SENSOR, "addr: 0x%x data: 0x%x dt: %d", addr, data,
		data_type);
	...
	for (i = 0; i < delay_ms; i++) {
		rc = cam_cci_i2c_compare(client, addr, data, data_mask,
					 data_type, addr_type);
		if (!rc)
			return rc;
