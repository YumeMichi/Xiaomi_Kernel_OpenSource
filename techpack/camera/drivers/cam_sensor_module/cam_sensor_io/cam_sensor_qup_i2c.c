...
static int32_t cam_qup_i2c_rxdata(struct i2c_client *dev_client,
...
static int32_t cam_qup_i2c_txdata(struct camera_io_master *dev_client,
...
int32_t cam_qup_i2c_read(struct i2c_client *client, uint32_t addr,
			 uint32_t *data, enum camera_sensor_i2c_type addr_type,
...
int32_t cam_qup_i2c_read_seq(struct i2c_client *client, uint32_t addr,
			     uint8_t *data,
			     enum camera_sensor_i2c_type addr_type,
			     uint32_t num_byte)
{
...
static int32_t cam_qup_i2c_compare(struct i2c_client *client, uint32_t addr,
				   uint32_t data, uint16_t data_mask,
				   enum camera_sensor_i2c_type data_type,
				   enum camera_sensor_i2c_type addr_type)
{
	int32_t rc;
	...
	rc = cam_qup_i2c_read(client, addr, &reg_data, addr_type, data_type);
	if (rc < 0)
		return rc;
...
int32_t cam_qup_i2c_poll(struct i2c_client *client, uint32_t addr,
			 uint16_t data, uint16_t data_mask,
			 enum camera_sensor_i2c_type addr_type,
			 enum camera_sensor_i2c_type data_type,
			 uint32_t delay_ms)
{
	...
	for (i = 0; i < delay_ms; i++) {
		rc = cam_qup_i2c_compare(client, addr, data, data_mask,
					 data_type, addr_type);
...
int32_t
cam_qup_i2c_write_table(struct camera_io_master *client,
...
static int32_t
cam_qup_i2c_write_seq(struct camera_io_master *client,
		      struct cam_sensor_i2c_reg_setting *write_setting)
{
	int i;
	int32_t rc = 0;
...
static int32_t
cam_qup_i2c_write_burst(struct camera_io_master *client,
			struct cam_sensor_i2c_reg_setting *write_setting)
{
	int i;
	int32_t rc = 0;
	...
	for (i = 0; i < write_setting->size; i++) {
		if (data_type == CAMERA_SENSOR_I2C_TYPE_BYTE) {
			...
			buf[len + 1] = reg_setting->reg_data;
			CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len, buf[len]);
			CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len + 1,
				buf[len + 1]);
			len += 2;
		} else if (data_type == CAMERA_SENSOR_I2C_TYPE_3B) {
			buf[len] = reg_setting->reg_data >> 16;
			...
			CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len + 1,
				buf[len + 1]);
			CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len + 2,
				buf[len + 2]);
			len += 3;
		} else if (data_type == CAMERA_SENSOR_I2C_TYPE_DWORD) {
			buf[len] = reg_setting->reg_data >> 24;
			...
			CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len + 2,
				buf[len + 2]);
			CAM_DBG(CAM_SENSOR, "Byte %d: 0x%x", len + 3,
				buf[len + 3]);
			len += 4;
		} else {
			CAM_ERR(CAM_SENSOR, "Invalid Data Type");
