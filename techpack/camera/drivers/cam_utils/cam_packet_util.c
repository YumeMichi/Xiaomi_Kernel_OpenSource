				    ...
				    size_t remain_len)
{
	...
	if ((!packet->header.size) ||
	    ((pkt_wo_payload + (size_t)packet->cmd_buf_offset + sum_cmd_desc) >
	     (size_t)packet->header.size) ||
	    ((pkt_wo_payload + (size_t)packet->io_configs_offset +
	      sum_io_cfgs) > (size_t)packet->header.size) ||
	    ((pkt_wo_payload + (size_t)packet->patch_offset + sum_patch_desc) >
	     (size_t)packet->header.size)) {
		CAM_ERR(CAM_UTIL, "params not within mem len:%zu %zu %zu %zu",
			(size_t)packet->header.size, sum_cmd_desc, sum_io_cfgs,
			sum_patch_desc);
		return -EINVAL;
	}
				    ...
				    int32_t iommu_hdl, int32_t sec_mmu_hdl)
{
	...
	patch_desc = (struct cam_patch_desc *)((uint32_t *)&packet->payload +
	...
	for (i = 0; i < packet->num_patches; i++) {
		...
		rc = cam_mem_get_cpu_buf(patch_desc[i].dst_buf_hdl, &cpu_addr,
					 &dst_buf_len);
		...
		if ((size_t)patch_desc[i].src_offset >= src_buf_size) {
			CAM_ERR(CAM_UTIL, "Invalid src buf patch offset");
			return -EINVAL;
		}
		...
		if ((dst_buf_len < sizeof(void *)) ||
			...
			CAM_ERR(CAM_UTIL, "Invalid dst buf patch offset");
			return -EINVAL;
		}
	...
	cam_packet_generic_blob_handler blob_handler_cb, void *user_data)
{
	...
	if (!cmd_buf || !blob_handler_cb) {
		CAM_ERR(CAM_UTIL, "Invalid args %pK %pK", cmd_buf,
			blob_handler_cb);
		return -EINVAL;
	}

	if (!cmd_buf->length || !cmd_buf->size) {
		CAM_ERR(CAM_UTIL, "Invalid cmd buf size %d %d", cmd_buf->length,
			cmd_buf->size);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_UTIL, "GenericCmdBuffer cpuaddr=%pK, blobptr=%pK, len=%d",
	...
	while (len_read < cmd_buf->length) {
		...
		if (len_read + blob_block_size > cmd_buf->length) {
			CAM_ERR(CAM_UTIL, "Invalid Blob %d %d %d %d", blob_type,
				blob_size, len_read, cmd_buf->length);
			rc = -EINVAL;
			goto end;
		}
